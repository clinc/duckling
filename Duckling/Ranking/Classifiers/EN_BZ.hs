-- Copyright (c) 2016-present, Facebook, Inc.
-- All rights reserved.
--
-- This source code is licensed under the BSD-style license found in the
-- LICENSE file in the root directory of this source tree. An additional grant
-- of patent rights can be found in the PATENTS file in the same directory.

-----------------------------------------------------------------
-- Auto-generated by regenClassifiers
--
-- DO NOT EDIT UNLESS YOU ARE SURE THAT YOU KNOW WHAT YOU ARE DOING
--  @generated
-----------------------------------------------------------------
{-# LANGUAGE OverloadedStrings #-}
module Duckling.Ranking.Classifiers.EN_BZ (classifiers) where
import Data.String
import Prelude
import qualified Data.HashMap.Strict as HashMap
import Duckling.Ranking.Types

classifiers :: Classifiers
classifiers
  = HashMap.fromList
      [("Bhai Dooj",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<integer> to|till|before <hour-of-day>",
        Classifier{okData =
                     ClassData{prior = -2.5649493574615367, unseen = -1.791759469228055,
                               likelihoods =
                                 HashMap.fromList
                                   [("integer (numeric)noon|midnight|EOD|end of day",
                                     -0.916290731874155),
                                    ("hour", -0.916290731874155)],
                               n = 1},
                   koData =
                     ClassData{prior = -8.004270767353637e-2,
                               unseen = -3.332204510175204,
                               likelihoods =
                                 HashMap.fromList
                                   [("hour", -0.7308875085427924),
                                    ("integer (numeric)time-of-day (latent)", -0.7308875085427924)],
                               n = 12}}),
       ("week",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<time> timezone",
        Classifier{okData =
                     ClassData{prior = -9.237332013101517e-2,
                               unseen = -4.23410650459726,
                               likelihoods =
                                 HashMap.fromList
                                   [("at <time-of-day>", -2.4277482359480516),
                                    ("hhhmm", -1.65455834771457),
                                    ("<time-of-day> am|pm", -2.0222831278398874),
                                    ("hh:mm", -2.2735975561207935), ("hour", -1.821612432377736),
                                    ("minute", -1.128465251817791)],
                               n = 31},
                   koData =
                     ClassData{prior = -2.4277482359480516,
                               unseen = -2.5649493574615367,
                               likelihoods =
                                 HashMap.fromList
                                   [("hhhmm", -1.3862943611198906), ("hh:mm", -1.791759469228055),
                                    ("minute", -1.0986122886681098)],
                               n = 3}}),
       ("Thursday",
        Classifier{okData =
                     ClassData{prior = -7.79615414697118e-2,
                               unseen = -3.6635616461296463,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 37},
                   koData =
                     ClassData{prior = -2.5902671654458267,
                               unseen = -1.6094379124341003,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 3}}),
       ("integer (numeric)",
        Classifier{okData =
                     ClassData{prior = -0.5212969236332861, unseen = -6.244166900663736,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 513},
                   koData =
                     ClassData{prior = -0.9007865453381898,
                               unseen = -5.8664680569332965,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 351}}),
       ("<year> (bc|ad)",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.9459101490553135,
                               likelihoods = HashMap.fromList [("integer (numeric)", 0.0)],
                               n = 5},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<duration> hence|ago",
        Classifier{okData =
                     ClassData{prior = -0.13353139262452263,
                               unseen = -3.951243718581427,
                               likelihoods =
                                 HashMap.fromList
                                   [("week", -1.7346010553881064), ("second", -3.2386784521643803),
                                    ("day", -1.8523840910444898), ("year", -2.833213344056216),
                                    ("<integer> <unit-of-duration>", -1.0986122886681098),
                                    ("a <unit-of-duration>", -2.5455312716044354),
                                    ("month", -2.833213344056216),
                                    ("fortnight", -2.833213344056216),
                                    ("minute", -3.2386784521643803)],
                               n = 21},
                   koData =
                     ClassData{prior = -2.0794415416798357, unseen = -2.772588722239781,
                               likelihoods =
                                 HashMap.fromList
                                   [("day", -2.0149030205422647), ("year", -1.6094379124341003),
                                    ("<integer> <unit-of-duration>", -1.3217558399823195)],
                               n = 3}}),
       ("noon|midnight|EOD|end of day",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.3978952727983707,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 9},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("quarter to|till|before <hour-of-day>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.6094379124341003,
                               likelihoods =
                                 HashMap.fromList
                                   [("noon|midnight|EOD|end of day", -0.6931471805599453),
                                    ("hour", -0.6931471805599453)],
                               n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Karva Chauth",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<named-month>|<named-day> <day-of-month> the (ordinal)",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.639057329615259,
                               likelihoods =
                                 HashMap.fromList
                                   [("day", -1.8718021769015913),
                                    ("Marchordinals (first..twentieth,thirtieth,...)",
                                     -1.8718021769015913),
                                    ("Februaryordinal (digits)", -1.466337068793427),
                                    ("Tuesdayordinal (digits)", -1.8718021769015913),
                                    ("month", -1.1786549963416462)],
                               n = 4},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.791759469228055,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<cycle> after|before <time>",
        Classifier{okData =
                     ClassData{prior = -0.6931471805599453, unseen = -2.833213344056216,
                               likelihoods =
                                 HashMap.fromList
                                   [("day (grain)tomorrow", -1.6739764335716716),
                                    ("dayday", -1.1631508098056809),
                                    ("day (grain)yesterday", -1.6739764335716716)],
                               n = 4},
                   koData =
                     ClassData{prior = -0.6931471805599453, unseen = -2.833213344056216,
                               likelihoods =
                                 HashMap.fromList
                                   [("dayhour", -1.6739764335716716),
                                    ("year (grain)Christmas", -2.0794415416798357),
                                    ("dayday", -2.0794415416798357),
                                    ("day (grain)intersect", -1.6739764335716716),
                                    ("day (grain)Easter Sunday", -2.0794415416798357),
                                    ("yearday", -2.0794415416798357)],
                               n = 4}}),
       ("Easter Monday",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Navaratri",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Martin Luther King's Day",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.4849066497880004,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 10},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("integer (20..90)",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.1972245773362196,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 7},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Shemini Atzeret",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("in <duration> at <time-of-day>",
        Classifier{okData =
                     ClassData{prior = -0.6931471805599453,
                               unseen = -2.1972245773362196,
                               likelihoods =
                                 HashMap.fromList
                                   [("dayhour", -1.3862943611198906),
                                    ("yearhour", -1.3862943611198906),
                                    ("<integer> <unit-of-duration><time-of-day> am|pm",
                                     -0.9808292530117262)],
                               n = 2},
                   koData =
                     ClassData{prior = -0.6931471805599453,
                               unseen = -2.1972245773362196,
                               likelihoods =
                                 HashMap.fromList
                                   [("dayhour", -1.3862943611198906),
                                    ("yearhour", -1.3862943611198906),
                                    ("<integer> <unit-of-duration>time-of-day (latent)",
                                     -0.9808292530117262)],
                               n = 2}}),
       ("Maha Shivaratri",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Ramadan",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.0794415416798357,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 6},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Lazarus Saturday",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("intersect 2 numbers",
        Classifier{okData =
                     ClassData{prior = -0.6931471805599453, unseen = -1.791759469228055,
                               likelihoods =
                                 HashMap.fromList
                                   [("compose by multiplicationinteger (0..19)",
                                     -0.2231435513142097)],
                               n = 3},
                   koData =
                     ClassData{prior = -0.6931471805599453, unseen = -1.791759469228055,
                               likelihoods =
                                 HashMap.fromList
                                   [("powers of tensinteger (0..19)", -0.2231435513142097)],
                               n = 3}}),
       ("mm/yyyy",
        Classifier{okData =
                     ClassData{prior = -0.6931471805599453,
                               unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -0.6931471805599453,
                               unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1}}),
       ("integer after|past <hour-of-day>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.5649493574615367,
                               likelihoods =
                                 HashMap.fromList
                                   [("integer (numeric)noon|midnight|EOD|end of day",
                                     -1.791759469228055),
                                    ("hour", -0.8754687373538999),
                                    ("integer (numeric)time-of-day (latent)", -1.3862943611198906),
                                    ("integer (20..90)time-of-day (latent)", -1.791759469228055)],
                               n = 4},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.6094379124341003,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<ordinal> last <cycle> of <time>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -3.4011973816621555,
                               likelihoods =
                                 HashMap.fromList
                                   [("ordinals (first..twentieth,thirtieth,...)week (grain)year (latent)",
                                     -2.268683541318364),
                                    ("daymonth", -2.268683541318364),
                                    ("ordinal (digits)day (grain)May", -2.6741486494265287),
                                    ("ordinals (first..twentieth,thirtieth,...)week (grain)intersect",
                                     -2.6741486494265287),
                                    ("weekmonth", -1.7578579175523736),
                                    ("ordinal (digits)week (grain)October", -2.6741486494265287),
                                    ("ordinal (digits)week (grain)intersect", -2.6741486494265287),
                                    ("ordinal (digits)week (grain)year (latent)",
                                     -2.6741486494265287),
                                    ("weekyear", -1.9810014688665833),
                                    ("ordinals (first..twentieth,thirtieth,...)week (grain)October",
                                     -2.6741486494265287),
                                    ("ordinals (first..twentieth,thirtieth,...)day (grain)May",
                                     -2.6741486494265287)],
                               n = 9},
                   koData =
                     ClassData{prior = -infinity, unseen = -2.4849066497880004,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Yom HaShoah",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<day-of-month> (ordinal or number) <named-month>",
        Classifier{okData =
                     ClassData{prior = -1.2237754316221157,
                               unseen = -3.0910424533583156,
                               likelihoods =
                                 HashMap.fromList
                                   [("ordinal (digits)December", -1.6582280766035324),
                                    ("ordinal (digits)February", -2.3513752571634776),
                                    ("integer (numeric)April", -2.3513752571634776),
                                    ("month", -1.252762968495368)],
                               n = 5},
                   koData =
                     ClassData{prior = -0.3483066942682157, unseen = -3.58351893845611,
                               likelihoods =
                                 HashMap.fromList
                                   [("ordinal (digits)October", -2.8622008809294686),
                                    ("ordinal (digits)July", -2.169053700369523),
                                    ("integer (numeric)September", -2.456735772821304),
                                    ("ordinal (digits)August", -2.8622008809294686),
                                    ("ordinal (digits)April", -2.8622008809294686),
                                    ("month", -0.9903987040278769),
                                    ("integer (numeric)July", -2.169053700369523),
                                    ("integer (numeric)until <time>", -2.8622008809294686)],
                               n = 12}}),
       ("<time> <part-of-day>",
        Classifier{okData =
                     ClassData{prior = -0.19574457712609536,
                               unseen = -4.624972813284271,
                               likelihoods =
                                 HashMap.fromList
                                   [("<day-of-month> (ordinal)in|during the <part-of-day>",
                                     -3.9219733362813143),
                                    ("dayhour", -1.4796263009121098),
                                    ("Mondayearly morning", -3.5165082281731497),
                                    ("<named-month>|<named-day> <day-of-month> the (ordinal)in|during the <part-of-day>",
                                     -3.9219733362813143),
                                    ("time-of-day (latent)tonight", -3.5165082281731497),
                                    ("hourhour", -2.312535423847214),
                                    ("<time-of-day> o'clockin|during the <part-of-day>",
                                     -3.9219733362813143),
                                    ("todaypart of days", -3.9219733362813143),
                                    ("minutehour", -2.6692103677859462),
                                    ("at <time-of-day>in|during the <part-of-day>",
                                     -3.5165082281731497),
                                    ("time-of-day (latent)this <part-of-day>", -3.9219733362813143),
                                    ("Mondayin|during the <part-of-day>", -3.9219733362813143),
                                    ("intersectpart of days", -3.228826155721369),
                                    ("Saturdaypart of days", -3.9219733362813143),
                                    ("<day-of-month> (ordinal or number) of <named-month>in|during the <part-of-day>",
                                     -3.9219733362813143),
                                    ("the <day-of-month> (ordinal)in|during the <part-of-day>",
                                     -3.9219733362813143),
                                    ("tomorrowpart of days", -2.312535423847214),
                                    ("hh:mmin|during the <part-of-day>", -3.005682604407159),
                                    ("time-of-day (latent)in|during the <part-of-day>",
                                     -3.9219733362813143),
                                    ("hhmm (latent)in|during the <part-of-day>",
                                     -3.9219733362813143),
                                    ("yesterdaypart of days", -3.5165082281731497),
                                    ("<day-of-month> (ordinal or number) of <month>in|during the <part-of-day>",
                                     -3.9219733362813143),
                                    ("Mondaypart of days", -3.9219733362813143)],
                               n = 37},
                   koData =
                     ClassData{prior = -1.7272209480904839, unseen = -3.784189633918261,
                               likelihoods =
                                 HashMap.fromList
                                   [("dayhour", -3.068052935133617),
                                    ("yearhour", -3.068052935133617),
                                    ("monthhour", -3.068052935133617),
                                    ("hourhour", -1.9694406464655074),
                                    ("at <time-of-day>in|during the <part-of-day>",
                                     -3.068052935133617),
                                    ("year (latent)in|during the <part-of-day>",
                                     -3.068052935133617),
                                    ("Februaryin|during the <part-of-day>", -3.068052935133617),
                                    ("tomorrowpart of days", -3.068052935133617),
                                    ("time-of-day (latent)in|during the <part-of-day>",
                                     -2.151762203259462)],
                               n = 8}}),
       ("dd/mm",
        Classifier{okData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0},
                   koData =
                     ClassData{prior = 0.0, unseen = -3.4011973816621555,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 28}}),
       ("the last <cycle> of <time>",
        Classifier{okData =
                     ClassData{prior = -0.916290731874155, unseen = -3.1354942159291497,
                               likelihoods =
                                 HashMap.fromList
                                   [("day (grain)October", -1.9924301646902063),
                                    ("daymonth", -1.4816045409242156),
                                    ("day (grain)intersect", -1.9924301646902063),
                                    ("weekmonth", -1.9924301646902063),
                                    ("week (grain)intersect", -2.3978952727983707),
                                    ("week (grain)September", -2.3978952727983707)],
                               n = 6},
                   koData =
                     ClassData{prior = -0.5108256237659907, unseen = -3.367295829986474,
                               likelihoods =
                                 HashMap.fromList
                                   [("daymonth", -2.2335922215070942),
                                    ("day (grain)May", -2.2335922215070942),
                                    ("week (grain)year (latent)", -1.9459101490553135),
                                    ("weekmonth", -1.7227665977411035),
                                    ("week (grain)October", -2.2335922215070942),
                                    ("weekyear", -1.9459101490553135),
                                    ("week (grain)intersect", -2.2335922215070942)],
                               n = 9}}),
       ("today",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.3978952727983707,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 9},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("at <time-of-day>",
        Classifier{okData =
                     ClassData{prior = -0.22957444164450025,
                               unseen = -5.308267697401205,
                               likelihoods =
                                 HashMap.fromList
                                   [("<time> timezone", -3.917010546939185),
                                    ("noon|midnight|EOD|end of day", -4.204692619390966),
                                    ("integer after|past <hour-of-day>", -3.917010546939185),
                                    ("<time-of-day> o'clock", -4.61015772749913),
                                    ("half after|past <hour-of-day>", -4.61015772749913),
                                    ("hhhmm", -3.6938669956249752),
                                    ("<hour-of-day> <integer>", -3.917010546939185),
                                    ("time-of-day (latent)", -1.6397432619294292),
                                    ("hhmm (latent)", -3.917010546939185),
                                    ("<time-of-day> am|pm", -2.0074680420547466),
                                    ("hh:mm", -3.3573947590037623),
                                    ("about|exactly <time-of-day>", -4.204692619390966),
                                    ("hour", -1.11365016603265),
                                    ("<time-of-day> sharp|exactly", -4.61015772749913),
                                    ("minute", -1.9360090780726016)],
                               n = 93},
                   koData =
                     ClassData{prior = -1.5841201044498106,
                               unseen = -4.1588830833596715,
                               likelihoods =
                                 HashMap.fromList
                                   [("time-of-day (latent)", -1.0076405104623831),
                                    ("<time-of-day> am|pm", -3.044522437723423),
                                    ("hour", -0.924258901523332)],
                               n = 24}}),
       ("December",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.9459101490553135,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 5},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("absorption of , after named day",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -3.58351893845611,
                               likelihoods =
                                 HashMap.fromList
                                   [("Wednesday", -2.8622008809294686),
                                    ("Saturday", -2.8622008809294686),
                                    ("Monday", -2.456735772821304), ("Friday", -1.6094379124341003),
                                    ("day", -0.8472978603872037), ("Sunday", -2.8622008809294686),
                                    ("on <day>", -2.169053700369523)],
                               n = 14},
                   koData =
                     ClassData{prior = -infinity, unseen = -2.0794415416798357,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("September",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.1972245773362196,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 7},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("tonight",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.4849066497880004,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 10},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("last|past|next <duration>",
        Classifier{okData =
                     ClassData{prior = -3.17486983145803e-2,
                               unseen = -4.2626798770413155,
                               likelihoods =
                                 HashMap.fromList
                                   [("week", -2.456735772821304), ("second", -2.639057329615259),
                                    ("day", -2.3025850929940455), ("year", -2.639057329615259),
                                    ("<integer> <unit-of-duration>", -0.7827593392496325),
                                    ("hour", -2.639057329615259), ("month", -2.639057329615259),
                                    ("minute", -2.639057329615259)],
                               n = 31},
                   koData =
                     ClassData{prior = -3.4657359027997265,
                               unseen = -2.3978952727983707,
                               likelihoods =
                                 HashMap.fromList
                                   [("<integer> <unit-of-duration>", -1.6094379124341003),
                                    ("hour", -1.6094379124341003)],
                               n = 1}}),
       ("the ides of <named-month>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.6094379124341003,
                               likelihoods =
                                 HashMap.fromList
                                   [("March", -0.6931471805599453), ("month", -0.6931471805599453)],
                               n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("the <day-of-month> (ordinal or number) of <named-month>",
        Classifier{okData =
                     ClassData{prior = -0.5978370007556204,
                               unseen = -3.4011973816621555,
                               likelihoods =
                                 HashMap.fromList
                                   [("ordinal (digits)July", -2.6741486494265287),
                                    ("ordinal (digits)January", -2.6741486494265287),
                                    ("ordinals (first..twentieth,thirtieth,...)March",
                                     -1.7578579175523736),
                                    ("ordinal (digits)February", -1.9810014688665833),
                                    ("month", -0.8823891801984737),
                                    ("ordinal (digits)March", -2.268683541318364)],
                               n = 11},
                   koData =
                     ClassData{prior = -0.7985076962177716, unseen = -3.258096538021482,
                               likelihoods =
                                 HashMap.fromList
                                   [("ordinal (digits)July", -1.4271163556401458),
                                    ("month", -0.916290731874155),
                                    ("integer (numeric)July", -1.6094379124341003)],
                               n = 9}}),
       ("integer (0..19)",
        Classifier{okData =
                     ClassData{prior = -1.227009259181436e-2,
                               unseen = -4.418840607796598,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 81},
                   koData =
                     ClassData{prior = -4.406719247264253, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1}}),
       ("in|during <named-month>|year",
        Classifier{okData =
                     ClassData{prior = -0.21511137961694557, unseen = -4.04305126783455,
                               likelihoods =
                                 HashMap.fromList
                                   [("<year> (bc|ad)", -2.2335922215070942),
                                    ("October", -3.332204510175204),
                                    ("year (latent)", -1.252762968495368),
                                    ("year", -0.9808292530117262), ("March", -2.4159137783010487),
                                    ("month", -2.2335922215070942)],
                               n = 25},
                   koData =
                     ClassData{prior = -1.6422277352570913,
                               unseen = -2.9444389791664407,
                               likelihoods =
                                 HashMap.fromList
                                   [("October", -1.5040773967762742),
                                    ("year (latent)", -1.5040773967762742),
                                    ("year", -1.5040773967762742), ("month", -1.5040773967762742)],
                               n = 6}}),
       ("<part-of-day> at <time-of-day>",
        Classifier{okData =
                     ClassData{prior = -8.223809823697212e-2,
                               unseen = -4.406719247264253,
                               likelihoods =
                                 HashMap.fromList
                                   [("this <part-of-day>hh:mm", -3.7013019741124937),
                                    ("tonighthh:mm", -3.7013019741124937),
                                    ("hourhour", -0.8979415932059586),
                                    ("hourminute", -3.0081547935525483),
                                    ("in|during the <part-of-day>time-of-day (latent)",
                                     -3.7013019741124937),
                                    ("this <part-of-day>time-of-day (latent)", -1.9965538818740682),
                                    ("early morningtime-of-day (latent)", -3.7013019741124937),
                                    ("tonight<time-of-day> o'clock", -3.7013019741124937),
                                    ("tonighttime-of-day (latent)", -2.7850112422383386),
                                    ("part of dayshh:mm", -3.7013019741124937),
                                    ("part of daystime-of-day (latent)", -1.6218604324326575)],
                               n = 35},
                   koData =
                     ClassData{prior = -2.538973871058276, unseen = -2.890371757896165,
                               likelihoods =
                                 HashMap.fromList
                                   [("hourhour", -1.4469189829363254),
                                    ("this <part-of-day>time-of-day (latent)", -2.1400661634962708),
                                    ("tonighttime-of-day (latent)", -2.1400661634962708),
                                    ("part of daystime-of-day (latent)", -2.1400661634962708)],
                               n = 3}}),
       ("between <time-of-day> and <time-of-day> (interval)",
        Classifier{okData =
                     ClassData{prior = -0.40546510810816444,
                               unseen = -2.833213344056216,
                               likelihoods =
                                 HashMap.fromList
                                   [("minuteminute", -1.6739764335716716),
                                    ("hh:mmhh:mm", -2.0794415416798357),
                                    ("<time-of-day> am|pmtime-of-day (latent)",
                                     -2.0794415416798357),
                                    ("hhhmmhhhmm", -2.0794415416798357),
                                    ("minutehour", -1.6739764335716716),
                                    ("<time-of-day> am|pm<time-of-day> am|pm",
                                     -2.0794415416798357)],
                               n = 4},
                   koData =
                     ClassData{prior = -1.0986122886681098,
                               unseen = -2.5649493574615367,
                               likelihoods =
                                 HashMap.fromList
                                   [("hh:mmtime-of-day (latent)", -1.791759469228055),
                                    ("minutehour", -1.3862943611198906),
                                    ("hhhmmtime-of-day (latent)", -1.791759469228055)],
                               n = 2}}),
       ("Halloween",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.6094379124341003,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 3},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Passover",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("from <month> dd-dd (interval)",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.3978952727983707,
                               likelihoods =
                                 HashMap.fromList
                                   [("Julyinteger (numeric)integer (numeric)", -1.6094379124341003),
                                    ("Augustordinal (digits)integer (numeric)",
                                     -1.6094379124341003),
                                    ("month", -0.916290731874155),
                                    ("Augustordinal (digits)ordinal (digits)",
                                     -1.6094379124341003)],
                               n = 3},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.6094379124341003,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Good Friday",
        Classifier{okData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0},
                   koData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2}}),
       ("October",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -3.1780538303479458,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 22},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("month (grain)",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -3.713572066704308,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 39},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<integer> more <unit-of-duration>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.6094379124341003,
                               likelihoods =
                                 HashMap.fromList
                                   [("integer (numeric)minute (grain)", -0.6931471805599453),
                                    ("minute", -0.6931471805599453)],
                               n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<time-of-day> o'clock",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -3.044522437723423,
                               likelihoods =
                                 HashMap.fromList
                                   [("at <time-of-day>", -2.3025850929940455),
                                    ("<part-of-day> at <time-of-day>", -2.3025850929940455),
                                    ("time-of-day (latent)", -1.0498221244986778),
                                    ("hour", -0.7985076962177716)],
                               n = 8},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.6094379124341003,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Vesak",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.791759469228055,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 4},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Earth Hour",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("in|within|after <duration>",
        Classifier{okData =
                     ClassData{prior = -0.12260232209233239,
                               unseen = -4.709530201312334,
                               likelihoods =
                                 HashMap.fromList
                                   [("week", -3.3141860046725258),
                                    ("<integer> more <unit-of-duration>", -4.007333185232471),
                                    ("three-quarters of an hour", -3.0910424533583156),
                                    ("<integer> + '\"", -3.3141860046725258),
                                    ("number.number hours", -4.007333185232471),
                                    ("second", -3.6018680771243066),
                                    ("half a <time-grain>", -3.0910424533583156),
                                    ("day", -3.3141860046725258), ("year", -4.007333185232471),
                                    ("<integer> <unit-of-duration>", -1.6094379124341003),
                                    ("a <unit-of-duration>", -3.0910424533583156),
                                    ("quarter of an hour", -3.0910424533583156),
                                    ("hour", -2.503255788456197),
                                    ("about|exactly <duration>", -4.007333185232471),
                                    ("half an hour (abbrev).", -3.6018680771243066),
                                    ("<integer> and an half hour", -4.007333185232471),
                                    ("minute", -1.2992829841302609)],
                               n = 46},
                   koData =
                     ClassData{prior = -2.159484249353372, unseen = -3.4339872044851463,
                               likelihoods =
                                 HashMap.fromList
                                   [("day", -2.70805020110221), ("quarter", -2.70805020110221),
                                    ("year", -1.791759469228055),
                                    ("<integer> <unit-of-duration>", -1.791759469228055),
                                    ("a <unit-of-duration>", -2.3025850929940455)],
                               n = 6}}),
       ("the closest <day> to <time>",
        Classifier{okData =
                     ClassData{prior = -0.2876820724517809,
                               unseen = -2.5649493574615367,
                               likelihoods =
                                 HashMap.fromList
                                   [("dayday", -1.0986122886681098),
                                    ("Christmastoday", -1.791759469228055),
                                    ("Monday<named-month>|<named-day> <day-of-month> (ordinal)",
                                     -1.791759469228055),
                                    ("Monday<named-month> <day-of-month> (non ordinal)",
                                     -1.791759469228055)],
                               n = 3},
                   koData =
                     ClassData{prior = -1.3862943611198906,
                               unseen = -2.1972245773362196,
                               likelihoods =
                                 HashMap.fromList
                                   [("daymonth", -1.3862943611198906),
                                    ("MondayOctober", -1.3862943611198906)],
                               n = 1}}),
       ("January",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.9459101490553135,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 5},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("three-quarters of an hour",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.791759469228055,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 4},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Mattu Pongal",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Wednesday",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.3025850929940455,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 8},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("half after|past <hour-of-day>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.9459101490553135,
                               likelihoods =
                                 HashMap.fromList
                                   [("time-of-day (latent)", -0.6931471805599453),
                                    ("hour", -0.6931471805599453)],
                               n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<integer> + '\"",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.6094379124341003,
                               likelihoods = HashMap.fromList [("integer (numeric)", 0.0)],
                               n = 3},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("half <integer> (UK style hour-of-day)",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.6094379124341003,
                               likelihoods =
                                 HashMap.fromList
                                   [("time-of-day (latent)", -0.6931471805599453),
                                    ("hour", -0.6931471805599453)],
                               n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Ganesh Chaturthi",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("July",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -3.871201010907891,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 46},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<time-of-day> <part-of-day>",
        Classifier{okData =
                     ClassData{prior = -0.12516314295400605,
                               unseen = -3.6888794541139363,
                               likelihoods =
                                 HashMap.fromList
                                   [("time-of-day (latent)tonight", -2.5649493574615367),
                                    ("hourhour", -1.3609765531356008),
                                    ("<time-of-day> o'clockin|during the <part-of-day>",
                                     -2.9704144655697013),
                                    ("minutehour", -1.717651497074333),
                                    ("at <time-of-day>in|during the <part-of-day>",
                                     -2.277267285009756),
                                    ("time-of-day (latent)this <part-of-day>", -2.9704144655697013),
                                    ("hh:mmin|during the <part-of-day>", -2.0541237336955462),
                                    ("time-of-day (latent)in|during the <part-of-day>",
                                     -2.277267285009756),
                                    ("hhmm (latent)in|during the <part-of-day>",
                                     -2.9704144655697013)],
                               n = 15},
                   koData =
                     ClassData{prior = -2.1400661634962708, unseen = -2.639057329615259,
                               likelihoods =
                                 HashMap.fromList
                                   [("hourhour", -1.466337068793427),
                                    ("time-of-day (latent)in|during the <part-of-day>",
                                     -1.466337068793427)],
                               n = 2}}),
       ("the nth <time> of <time>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.1972245773362196,
                               likelihoods =
                                 HashMap.fromList
                                   [("daymonth", -0.6931471805599453),
                                    ("ordinals (first..twentieth,thirtieth,...)Mondaythis|last|next <cycle>",
                                     -0.6931471805599453)],
                               n = 3},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("hour (grain)",
        Classifier{okData =
                     ClassData{prior = -1.3723081191451507, unseen = -2.995732273553991,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 18},
                   koData =
                     ClassData{prior = -0.2923879634891936, unseen = -4.007333185232471,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 53}}),
       ("Parsi New Year",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.791759469228055,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 4},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Shavuot",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<day> <duration> hence|ago",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.6094379124341003,
                               likelihoods =
                                 HashMap.fromList
                                   [("dayyear", -0.6931471805599453),
                                    ("Diwali<integer> <unit-of-duration>", -0.6931471805599453)],
                               n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<ordinal> quarter",
        Classifier{okData =
                     ClassData{prior = -0.4700036292457356, unseen = -2.639057329615259,
                               likelihoods =
                                 HashMap.fromList
                                   [("ordinal (digits)quarter (grain)", -1.1786549963416462),
                                    ("ordinals (first..twentieth,thirtieth,...)quarter (grain)",
                                     -1.466337068793427),
                                    ("quarter", -0.7731898882334817)],
                               n = 5},
                   koData =
                     ClassData{prior = -0.9808292530117262,
                               unseen = -2.3025850929940455,
                               likelihoods =
                                 HashMap.fromList
                                   [("ordinal (digits)quarter (grain)", -0.8109302162163288),
                                    ("quarter", -0.8109302162163288)],
                               n = 3}}),
       ("Boss's Day",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.0794415416798357,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 6},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Orthodox Easter Sunday",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("one twenty two",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("May",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -3.295836866004329,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 25},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("intersect",
        Classifier{okData =
                     ClassData{prior = -0.38029723640123764,
                               unseen = -7.062191632286556,
                               likelihoods =
                                 HashMap.fromList
                                   [("Navaratriyear (latent)", -6.368187186350492),
                                    ("Karva Chauthyear (latent)", -6.368187186350492),
                                    ("Maha Shivaratriyear (latent)", -6.368187186350492),
                                    ("Ramadanyear (latent)", -5.115424217855124),
                                    ("<datetime> - <datetime> (interval)on <day>",
                                     -5.269574897682382),
                                    ("Bhai Doojyear (latent)", -6.368187186350492),
                                    ("hourday", -4.422277037295179),
                                    ("dayhour", -3.534973842294276),
                                    ("<time-of-day> - <time-of-day> (interval)on <day>",
                                     -5.269574897682382),
                                    ("Martin Luther King's Dayyear (latent)", -5.962722078242328),
                                    ("Shemini Atzeretyear (latent)", -6.368187186350492),
                                    ("daymonth", -5.115424217855124),
                                    ("monthyear", -3.970291913552122),
                                    ("Yom Ha'atzmautyear (latent)", -6.368187186350492),
                                    ("Orthodox Good Fridayyear (latent)", -5.962722078242328),
                                    ("Vijayadashamiyear (latent)", -6.368187186350492),
                                    ("Thai Pongalyear (latent)", -5.675040005790547),
                                    ("Thiru Onamyear (latent)", -5.675040005790547),
                                    ("hhhmmabsorption of , after named day", -6.368187186350492),
                                    ("from <datetime> - <datetime> (interval)July",
                                     -5.451896454476337),
                                    ("Krishna Janmashtamiyear (latent)", -5.962722078242328),
                                    ("Guru Gobind Singh Jayantiyear (latent)", -6.368187186350492),
                                    ("houryear", -5.675040005790547),
                                    ("this|next <day-of-week>hh(:mm) - <time-of-day> am|pm",
                                     -6.368187186350492),
                                    ("Christmas<time-of-day> am|pm", -6.368187186350492),
                                    ("last <day-of-week> of <time>year (latent)",
                                     -6.368187186350492),
                                    ("<time-of-day> am|pmintersect by \",\", \"of\", \"from\", \"'s\"",
                                     -5.675040005790547),
                                    ("<time-of-day> am|pmintersect", -5.115424217855124),
                                    ("Earth Houryear (latent)", -6.368187186350492),
                                    ("Ganesh Chaturthiyear (latent)", -6.368187186350492),
                                    ("Octoberyear (latent)", -4.8641097895742185),
                                    ("intersect<time-of-day> am|pm", -6.368187186350492),
                                    ("Mattu Pongalyear (latent)", -6.368187186350492),
                                    ("Saturday<part-of-day> at <time-of-day>", -6.368187186350492),
                                    ("Januaryyear (latent)", -5.962722078242328),
                                    ("Shavuotyear (latent)", -6.368187186350492),
                                    ("Parsi New Yearyear (latent)", -5.451896454476337),
                                    ("at <time-of-day>in|within|after <duration>",
                                     -6.368187186350492),
                                    ("Thursdayhh(:mm) - <time-of-day> am|pm", -6.368187186350492),
                                    ("todayin|within|after <duration>", -6.368187186350492),
                                    ("<named-month>|<named-day> <day-of-month> (ordinal)year (latent)",
                                     -5.962722078242328),
                                    ("Marchyear (latent)", -5.962722078242328),
                                    ("intersect by \",\", \"of\", \"from\", \"'s\"hhhmm",
                                     -5.962722078242328),
                                    ("<named-month>|<named-day> <day-of-month> the (ordinal)in|during the <part-of-day>",
                                     -6.368187186350492),
                                    ("Sukkotyear (latent)", -5.962722078242328),
                                    ("hhhmmintersect", -5.962722078242328),
                                    ("intersect by \",\", \"of\", \"from\", \"'s\"year (latent)",
                                     -6.368187186350492),
                                    ("since <time>year (latent)", -6.368187186350492),
                                    ("<time-of-day> o'clock<time> <part-of-day>",
                                     -5.962722078242328),
                                    ("Clean Mondayyear (latent)", -5.962722078242328),
                                    ("monthhour", -6.368187186350492),
                                    ("todayat <time-of-day>", -6.368187186350492),
                                    ("Thursday<time> timezone", -3.970291913552122),
                                    ("tonight<time-of-day> am|pm", -6.368187186350492),
                                    ("<time-of-day> am|pmon <day>", -4.353284165808228),
                                    ("this <time>hh(:mm) - <time-of-day> am|pm",
                                     -6.368187186350492),
                                    ("yyyy-mm-ddhh:mm:ss", -5.962722078242328),
                                    ("dayday", -4.016811929187015),
                                    ("<time> <part-of-day>at <time-of-day>", -5.962722078242328),
                                    ("tonightat <time-of-day>", -5.269574897682382),
                                    ("<time-of-day> am|pmabsorption of , after named day",
                                     -5.675040005790547),
                                    ("Dayananda Saraswati Jayantiyear (latent)",
                                     -6.368187186350492),
                                    ("today<time-of-day> am|pm", -6.368187186350492),
                                    ("at <time-of-day><time> <part-of-day>", -6.368187186350492),
                                    ("hourhour", -3.925840150981288),
                                    ("Mahavir Jayantiyear (latent)", -5.962722078242328),
                                    ("Navaratriin|during <named-month>|year", -6.368187186350492),
                                    ("Wednesdaythis|last|next <cycle>", -6.368187186350492),
                                    ("Lentyear (latent)", -6.368187186350492),
                                    ("intersect<named-month> <day-of-month> (non ordinal)",
                                     -4.496385009448901),
                                    ("Boghiyear (latent)", -6.368187186350492),
                                    ("dayyear", -1.778130638172449),
                                    ("Karva Chauthin|during <named-month>|year",
                                     -6.368187186350492),
                                    ("<time-of-day> o'clocktomorrow", -5.962722078242328),
                                    ("<time-of-day> o'clockin|during the <part-of-day>",
                                     -6.368187186350492),
                                    ("Thursdayat <time-of-day>", -5.675040005790547),
                                    ("Islamic New Yearyear (latent)", -5.962722078242328),
                                    ("Laylat al-Qadryear (latent)", -5.451896454476337),
                                    ("Aprilyear (latent)", -6.368187186350492),
                                    ("<day-of-month> (ordinal or number) of <named-month>year (latent)",
                                     -5.962722078242328),
                                    ("Shrove Tuesdayyear (latent)", -6.368187186350492),
                                    ("intersect by \",\", \"of\", \"from\" for year<time-of-day> am|pm",
                                     -5.675040005790547),
                                    ("hourminute", -6.368187186350492),
                                    ("<time-of-day> am|pmtomorrow", -5.269574897682382),
                                    ("Yom Kippuryear (latent)", -6.368187186350492),
                                    ("<part-of-day> of <time>year (latent)", -6.368187186350492),
                                    ("minutehour", -5.269574897682382),
                                    ("Kaanum Pongalyear (latent)", -5.962722078242328),
                                    ("Maha Saptamiyear (latent)", -6.368187186350492),
                                    ("at <time-of-day>in|during the <part-of-day>",
                                     -5.675040005790547),
                                    ("absorption of , after named day<named-month> <day-of-month> (non ordinal)",
                                     -4.353284165808228),
                                    ("for <duration> from <time>December", -6.368187186350492),
                                    ("tomorrow<time-of-day> sharp|exactly", -6.368187186350492),
                                    ("Thursdayfrom <datetime> - <datetime> (interval)",
                                     -5.115424217855124),
                                    ("intersect by \",\", \"of\", \"from\" for yearhhhmm",
                                     -5.115424217855124),
                                    ("Pentecostyear (latent)", -6.368187186350492),
                                    ("Thursdayfrom <time-of-day> - <time-of-day> (interval)",
                                     -5.115424217855124),
                                    ("Eid al-Fitryear (latent)", -4.981892825230601),
                                    ("Vasant Panchamiin|during <named-month>|year",
                                     -6.368187186350492),
                                    ("Mondayin|during the <part-of-day>", -6.368187186350492),
                                    ("Chhathin|during <named-month>|year", -6.368187186350492),
                                    ("Diwaliin|during <named-month>|year", -6.368187186350492),
                                    ("this <part-of-day><time-of-day> am|pm", -6.368187186350492),
                                    ("Vaisakhiin|during <named-month>|year", -6.368187186350492),
                                    ("Guru Ravidass Jayantiyear (latent)", -5.115424217855124),
                                    ("Raksha Bandhanyear (latent)", -6.368187186350492),
                                    ("daysecond", -5.962722078242328),
                                    ("tomorrowfrom <time-of-day> - <time-of-day> (interval)",
                                     -5.962722078242328),
                                    ("Februaryyear (latent)", -6.368187186350492),
                                    ("Ratha-Yatrayear (latent)", -6.368187186350492),
                                    ("Ashurayear (latent)", -6.368187186350492),
                                    ("Tuesdayin|during <named-month>|year", -6.368187186350492),
                                    ("Chinese New Yearyear (latent)", -6.368187186350492),
                                    ("Juneyear (latent)", -5.962722078242328),
                                    ("Lag BaOmeryear (latent)", -6.368187186350492),
                                    ("last weekend of <named-month>year (latent)",
                                     -6.368187186350492),
                                    ("Eid al-Adhayear (latent)", -4.228121022854221),
                                    ("Palm Sundayyear (latent)", -6.368187186350492),
                                    ("Christmasat <time-of-day>", -6.368187186350492),
                                    ("Passoveryear (latent)", -6.368187186350492),
                                    ("Lazarus Saturdayyear (latent)", -6.368187186350492),
                                    ("<day-of-month> (ordinal or number) <named-month>year (latent)",
                                     -6.368187186350492),
                                    ("the last <cycle> of <time>year (latent)", -5.675040005790547),
                                    ("hhhmmon <day>", -5.451896454476337),
                                    ("Yom HaShoahyear (latent)", -5.962722078242328),
                                    ("<day-of-month> (ordinal or number) of <named-month>in|during the <part-of-day>",
                                     -6.368187186350492),
                                    ("Septemberyear (latent)", -5.962722078242328),
                                    ("Decemberyear (latent)", -5.962722078242328),
                                    ("Halloweenyear (latent)", -6.368187186350492),
                                    ("the <day-of-month> (ordinal or number) of <named-month>year (latent)",
                                     -5.962722078242328),
                                    ("<ordinal> last <cycle> of <time>year (latent)",
                                     -5.962722078242328),
                                    ("from <time-of-day> - <time-of-day> (interval)on <day>",
                                     -5.675040005790547),
                                    ("intersect by \",\", \"of\", \"from\", \"'s\"<time-of-day> am|pm",
                                     -6.368187186350492),
                                    ("at <time-of-day>intersect", -5.451896454476337),
                                    ("Rosh Hashanahyear (latent)", -5.675040005790547),
                                    ("Dhanterasyear (latent)", -6.368187186350492),
                                    ("Tu BiShvatyear (latent)", -6.368187186350492),
                                    ("Holiyear (latent)", -5.675040005790547),
                                    ("<time-of-day> - <time-of-day> (interval)tomorrow",
                                     -6.368187186350492),
                                    ("Holika Dahanyear (latent)", -5.675040005790547),
                                    ("at <time-of-day>intersect by \",\", \"of\", \"from\", \"'s\"",
                                     -5.962722078242328),
                                    ("dayminute", -3.211186765200379),
                                    ("Mawlidyear (latent)", -6.368187186350492),
                                    ("from <datetime> - <datetime> (interval)on <day>",
                                     -5.962722078242328),
                                    ("<datetime> - <datetime> (interval)tomorrow",
                                     -6.368187186350492),
                                    ("Jumu'atul-Widayear (latent)", -5.451896454476337),
                                    ("minuteday", -2.666885212237999),
                                    ("absorption of , after named dayintersect",
                                     -5.675040005790547),
                                    ("Orthodox Easter Sundayyear (latent)", -6.368187186350492),
                                    ("<ordinal> <cycle> of <time>year (latent)",
                                     -6.368187186350492),
                                    ("intersecthhhmm", -5.962722078242328),
                                    ("Boss's Dayyear (latent)", -5.962722078242328),
                                    ("hhhmmintersect by \",\", \"of\", \"from\", \"'s\"",
                                     -6.368187186350492),
                                    ("Global Youth Service Dayyear (latent)", -6.368187186350492),
                                    ("Dhanterasin|during <named-month>|year", -6.368187186350492),
                                    ("tonight<time-of-day> o'clock", -6.368187186350492),
                                    ("Tisha B'Avyear (latent)", -6.368187186350492),
                                    ("Isra and Mi'rajyear (latent)", -5.451896454476337),
                                    ("at <time-of-day>on <day>", -4.758749273916392),
                                    ("<day-of-month> (ordinal or number) of <month>year (latent)",
                                     -5.962722078242328),
                                    ("at <time-of-day>absorption of , after named day",
                                     -5.962722078242328),
                                    ("Christmasyear (latent)", -5.962722078242328),
                                    ("Naraka Chaturdashiyear (latent)", -5.962722078242328),
                                    ("Thai Pongalin|during <named-month>|year", -6.368187186350492),
                                    ("dayweek", -6.368187186350492),
                                    ("Easter Sundayyear (latent)", -5.675040005790547),
                                    ("between <time-of-day> and <time-of-day> (interval)on <day>",
                                     -5.962722078242328),
                                    ("weekyear", -5.269574897682382),
                                    ("King's Dayyear (latent)", -4.8641097895742185),
                                    ("hh:mmin|during the <part-of-day>", -5.451896454476337),
                                    ("<cycle> after|before <time><time-of-day> am|pm",
                                     -5.962722078242328),
                                    ("first|second|third|fourth|fifth <day-of-week> of <time>year (latent)",
                                     -5.675040005790547),
                                    ("Hanukkahyear (latent)", -5.675040005790547),
                                    ("Rama Navamiyear (latent)", -6.368187186350492),
                                    ("February<time> <part-of-day>", -6.368187186350492),
                                    ("Great Lentyear (latent)", -6.368187186350492),
                                    ("tomorrowat <time-of-day>", -5.675040005790547),
                                    ("tomorrow<part-of-day> at <time-of-day>", -5.962722078242328),
                                    ("Ugadiyear (latent)", -4.981892825230601),
                                    ("Vaisakhiyear (latent)", -5.675040005790547),
                                    ("absorption of , after named dayintersect by \",\", \"of\", \"from\" for year",
                                     -5.675040005790547),
                                    ("at <time-of-day>tomorrow", -5.962722078242328),
                                    ("tomorrow<time-of-day> am|pm", -6.368187186350492),
                                    ("<named-month> <day-of-month> (non ordinal)year (latent)",
                                     -6.368187186350492),
                                    ("Diwaliyear (latent)", -6.368187186350492),
                                    ("between <time> and <time>on <day>", -5.962722078242328),
                                    ("Black Fridayyear (latent)", -5.962722078242328),
                                    ("the <ordinal> last <cycle> of <time>year (latent)",
                                     -6.368187186350492),
                                    ("in|during the <part-of-day>at <time-of-day>",
                                     -6.368187186350492),
                                    ("Chhathyear (latent)", -5.962722078242328),
                                    ("Vasant Panchamiyear (latent)", -6.368187186350492),
                                    ("Rabindra Jayantiyear (latent)", -5.115424217855124),
                                    ("this <part-of-day>at <time-of-day>", -5.115424217855124),
                                    ("St Patrick's Dayyear (latent)", -6.368187186350492),
                                    ("Thursday<time> (timezone)", -6.368187186350492),
                                    ("<day-of-month> (ordinal or number) of <month>in|during the <part-of-day>",
                                     -6.368187186350492),
                                    ("the <day-of-month> (ordinal or number) of <month>year (latent)",
                                     -5.962722078242328),
                                    ("Pargat Diwasyear (latent)", -5.115424217855124),
                                    ("on <day><named-month> <day-of-month> (non ordinal)",
                                     -5.675040005790547),
                                    ("Simchat Torahyear (latent)", -6.368187186350492),
                                    ("minuteyear", -6.368187186350492)],
                               n = 456},
                   koData =
                     ClassData{prior = -1.1509319124395572, unseen = -6.517671272912275,
                               likelihoods =
                                 HashMap.fromList
                                   [("Thursdayhhhmm", -4.318968498706744),
                                    ("hourday", -4.724433606814909),
                                    ("dayhour", -3.4716706383195413),
                                    ("<time> timezoneyear (latent)", -5.1298987149230735),
                                    ("<time-of-day> - <time-of-day> (interval)on <day>",
                                     -4.906755163608864),
                                    ("after <time><time-of-day> am|pm", -5.823045895483019),
                                    ("daymonth", -3.3806988601138146),
                                    ("monthday", -4.906755163608864),
                                    ("monthyear", -4.570282926987651),
                                    ("<time-of-day> am|pmyear (latent)", -5.1298987149230735),
                                    ("Thai Pongalyear (latent)", -5.1298987149230735),
                                    ("intersecthh:mm", -5.823045895483019),
                                    ("from <datetime> - <datetime> (interval)July",
                                     -5.417580787374854),
                                    ("houryear", -4.906755163608864),
                                    ("from <time-of-day> - <time-of-day> (interval)July",
                                     -5.823045895483019),
                                    ("<time-of-day> am|pmintersect by \",\", \"of\", \"from\", \"'s\"",
                                     -5.1298987149230735),
                                    ("<time-of-day> am|pmintersect", -4.570282926987651),
                                    ("Octoberyear (latent)", -5.823045895483019),
                                    ("Good Fridayyear (latent)", -5.417580787374854),
                                    ("until <time>on <day>", -5.1298987149230735),
                                    ("todayin <number> (implicit minutes)", -5.823045895483019),
                                    ("July<day-of-month> (ordinal or number) <named-month>",
                                     -5.823045895483019),
                                    ("<named-month>|<named-day> <day-of-month> (ordinal)year (latent)",
                                     -5.417580787374854),
                                    ("this <part-of-day><time-of-day> - <time-of-day> (interval)",
                                     -5.823045895483019),
                                    ("hh:mmon <day>", -4.570282926987651),
                                    ("hhhmmintersect", -5.417580787374854),
                                    ("absorption of , after named dayJuly", -4.906755163608864),
                                    ("intersect by \",\", \"of\", \"from\", \"'s\"year (latent)",
                                     -4.906755163608864),
                                    ("Clean Mondayyear (latent)", -5.823045895483019),
                                    ("monthhour", -5.823045895483019),
                                    ("hourmonth", -4.906755163608864),
                                    ("todayat <time-of-day>", -5.823045895483019),
                                    ("since <time>until <time>", -5.823045895483019),
                                    ("hhhmmyear (latent)", -4.570282926987651),
                                    ("<time-of-day> am|pmon <day>", -4.570282926987651),
                                    ("Mondayyear (latent)", -5.1298987149230735),
                                    ("dayday", -5.823045895483019),
                                    ("on <day>September", -5.1298987149230735),
                                    ("hourhour", -4.570282926987651),
                                    ("Thursdaydd/mm", -5.823045895483019),
                                    ("dayyear", -2.9052751633987395),
                                    ("New Year's Dayyear (latent)", -4.906755163608864),
                                    ("Thursdayat <time-of-day>", -4.213607983048918),
                                    ("<integer> to|till|before <hour-of-day>September",
                                     -5.823045895483019),
                                    ("Aprilyear (latent)", -5.823045895483019),
                                    ("monthminute", -5.823045895483019),
                                    ("<time-of-day> am|pmtomorrow", -5.417580787374854),
                                    ("Thursdayhh:mm", -4.906755163608864),
                                    ("minutemonth", -3.7436043538031827),
                                    ("Augustyear (latent)", -5.417580787374854),
                                    ("week-endin|during <named-month>|year", -5.823045895483019),
                                    ("tomorrowfrom <time-of-day> - <time-of-day> (interval)",
                                     -5.823045895483019),
                                    ("tonight<time-of-day> - <time-of-day> (interval)",
                                     -5.823045895483019),
                                    ("Sundayyear (latent)", -4.906755163608864),
                                    ("Christmasat <time-of-day>", -5.823045895483019),
                                    ("the last <cycle> of <time>year (latent)", -5.417580787374854),
                                    ("hhhmmon <day>", -4.906755163608864),
                                    ("Julyuntil <time>", -5.417580787374854),
                                    ("yesterday<time-of-day> am|pm", -5.823045895483019),
                                    ("intersect by \",\", \"of\", \"from\", \"'s\"hh:mm",
                                     -5.823045895483019),
                                    ("in|during <named-month>|yearyear (latent)",
                                     -5.823045895483019),
                                    ("at <time-of-day>intersect", -4.906755163608864),
                                    ("hh:mmyear (latent)", -4.436751534363128),
                                    ("Holiyear (latent)", -5.823045895483019),
                                    ("until <time><time-of-day> am|pm", -5.823045895483019),
                                    ("at <time-of-day>intersect by \",\", \"of\", \"from\", \"'s\"",
                                     -5.417580787374854),
                                    ("dayminute", -3.220356210038635),
                                    ("yyyy-mm-ddhh:mm", -5.417580787374854),
                                    ("<named-month> <day-of-month> (non ordinal)through <time>",
                                     -5.823045895483019),
                                    ("since <time><day-of-month> (ordinal or number) <named-month>",
                                     -5.823045895483019),
                                    ("intersectSeptember", -3.951243718581427),
                                    ("minuteday", -2.7549929603494014),
                                    ("absorption of , after named dayintersect",
                                     -5.823045895483019),
                                    ("Februaryin|during the <part-of-day>", -5.823045895483019),
                                    ("<duration> after|before|from|past <time>December",
                                     -5.823045895483019),
                                    ("Saturdayyear (latent)", -5.823045895483019),
                                    ("hhhmmintersect by \",\", \"of\", \"from\", \"'s\"",
                                     -5.823045895483019),
                                    ("at <time-of-day>on <day>", -4.906755163608864),
                                    ("absorption of , after named daySeptember",
                                     -4.570282926987651),
                                    ("Naraka Chaturdashiyear (latent)", -5.823045895483019),
                                    ("Easter Sundayyear (latent)", -5.417580787374854),
                                    ("Thursday<time-of-day> am|pm", -4.724433606814909),
                                    ("weekyear", -5.417580787374854),
                                    ("after <time>year (latent)", -5.417580787374854),
                                    ("<named-month> <day-of-month> (non ordinal)until <time>",
                                     -5.417580787374854),
                                    ("yyyy-mm-dd<time-of-day> - <time-of-day> (interval)",
                                     -5.417580787374854),
                                    ("intersect by \",\", \"of\", \"from\" for yearhh:mm",
                                     -5.1298987149230735),
                                    ("absorption of , after named dayFebruary", -4.906755163608864),
                                    ("July<integer> to|till|before <hour-of-day>",
                                     -5.823045895483019),
                                    ("tomorrowat <time-of-day>", -5.823045895483019),
                                    ("Fridayin|during <named-month>|year", -5.823045895483019),
                                    ("<integer> to|till|before <hour-of-day>July",
                                     -5.417580787374854),
                                    ("tomorrow<time-of-day> am|pm", -5.823045895483019),
                                    ("<named-month> <day-of-month> (non ordinal)year (latent)",
                                     -5.417580787374854),
                                    ("Diwaliyear (latent)", -5.417580787374854),
                                    ("this <part-of-day>at <time-of-day>", -4.906755163608864),
                                    ("Fridayyear (latent)", -4.906755163608864),
                                    ("minuteyear", -3.682979731986748)],
                               n = 211}}),
       ("one eleven",
        Classifier{okData =
                     ClassData{prior = -1.3862943611198906,
                               unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -0.2876820724517809,
                               unseen = -1.6094379124341003,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 3}}),
       ("after lunch/work/school",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("early morning",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.791759469228055,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 4},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("in <number> (implicit minutes)",
        Classifier{okData =
                     ClassData{prior = -1.329135947279942, unseen = -2.4849066497880004,
                               likelihoods =
                                 HashMap.fromList
                                   [("integer (numeric)", -0.3184537311185346),
                                    ("integer (0..19)", -1.2992829841302609)],
                               n = 9},
                   koData =
                     ClassData{prior = -0.30748469974796055,
                               unseen = -3.332204510175204,
                               likelihoods =
                                 HashMap.fromList
                                   [("integer (numeric)", -0.20479441264601328),
                                    ("integer (0..19)", -1.6863989535702288)],
                               n = 25}}),
       ("<ordinal> <cycle> of <time>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.9444389791664407,
                               likelihoods =
                                 HashMap.fromList
                                   [("daymonth", -1.791759469228055),
                                    ("ordinal (digits)quarter (grain)year (latent)",
                                     -2.1972245773362196),
                                    ("quarteryear", -2.1972245773362196),
                                    ("ordinals (first..twentieth,thirtieth,...)day (grain)October",
                                     -2.1972245773362196),
                                    ("ordinal (digits)day (grain)this|last|next <cycle>",
                                     -2.1972245773362196),
                                    ("ordinals (first..twentieth,thirtieth,...)week (grain)intersect",
                                     -2.1972245773362196),
                                    ("weekmonth", -1.791759469228055),
                                    ("ordinals (first..twentieth,thirtieth,...)week (grain)October",
                                     -2.1972245773362196)],
                               n = 5},
                   koData =
                     ClassData{prior = -infinity, unseen = -2.1972245773362196,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("year (grain)",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -3.6888794541139363,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 38},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("from <datetime> - <datetime> (interval)",
        Classifier{okData =
                     ClassData{prior = -0.7975071958841881, unseen = -5.117993812416755,
                               likelihoods =
                                 HashMap.fromList
                                   [("daymonth", -3.7256934272366524),
                                    ("monthday", -3.16607763930123),
                                    ("at the beginning|end of <named-month>June",
                                     -3.7256934272366524),
                                    ("hh:mmtime-of-day (latent)", -4.013375499688434),
                                    ("the <day-of-month> (ordinal or number) of <month>the <day-of-month> (number)",
                                     -4.418840607796598),
                                    ("minuteminute", -3.5025498759224427),
                                    ("the <day-of-month> (number)the <day-of-month> (ordinal)",
                                     -4.418840607796598),
                                    ("<day-of-month> (ordinal)<day-of-month> (ordinal)",
                                     -4.013375499688434),
                                    ("the <day-of-month> (ordinal)the <day-of-month> (ordinal)",
                                     -4.013375499688434),
                                    ("<named-month>|<named-day> <day-of-month> (ordinal)<named-month>|<named-day> <day-of-month> (ordinal)",
                                     -4.418840607796598),
                                    ("hh:mmhh:mm", -3.5025498759224427),
                                    ("dayday", -1.7797832781813394),
                                    ("the <day-of-month> (ordinal)the <day-of-month> (number)",
                                     -4.418840607796598),
                                    ("the <day-of-month> (number)the <day-of-month> (number)",
                                     -4.013375499688434),
                                    ("<named-month>|<named-day> <day-of-month> (ordinal)<day-of-month> (ordinal)",
                                     -4.418840607796598),
                                    ("monthmonth", -4.013375499688434),
                                    ("<time-of-day> am|pmtime-of-day (latent)", -4.418840607796598),
                                    ("hourhour", -4.013375499688434),
                                    ("<named-month> <day-of-month> (non ordinal)<named-month> <day-of-month> (non ordinal)",
                                     -3.7256934272366524),
                                    ("the <day-of-month> (ordinal or number) of <named-month>the <day-of-month> (number)",
                                     -4.418840607796598),
                                    ("minutehour", -4.013375499688434),
                                    ("<day-of-month> (ordinal)the <day-of-month> (ordinal)",
                                     -4.418840607796598),
                                    ("the <day-of-month> (ordinal)<day-of-month> (ordinal)",
                                     -4.418840607796598),
                                    ("the <day-of-month> (ordinal or number) of <month>the <day-of-month> (ordinal)",
                                     -4.418840607796598),
                                    ("at the beginning|end of <named-month>at the beginning|end of <named-month>",
                                     -3.16607763930123),
                                    ("<time-of-day> am|pm<time-of-day> am|pm", -4.418840607796598),
                                    ("<named-month>|<named-day> <day-of-month> (ordinal)<named-month> <day-of-month> (non ordinal)",
                                     -4.418840607796598),
                                    ("yearday", -3.5025498759224427),
                                    ("the <day-of-month> (ordinal or number) of <named-month>the <day-of-month> (ordinal)",
                                     -4.418840607796598),
                                    ("year (latent)at the beginning|end of <named-month>",
                                     -3.5025498759224427),
                                    ("Mayat the beginning|end of <named-month>", -3.16607763930123),
                                    ("MayJune", -4.013375499688434),
                                    ("<day-of-month> (ordinal)the <day-of-month> (number)",
                                     -4.418840607796598)],
                               n = 50},
                   koData =
                     ClassData{prior = -0.5986563371390228, unseen = -5.241747015059643,
                               likelihoods =
                                 HashMap.fromList
                                   [("<day-of-month> (ordinal)the <day-of-month> (ordinal or number) of <month>",
                                     -4.137829674161839),
                                    ("hourday", -2.597384633214691),
                                    ("<named-month> <day-of-month> (non ordinal)July",
                                     -3.8501476017100584),
                                    ("dayhour", -3.2905318137746358),
                                    ("daymonth", -3.6270040503958487),
                                    ("<day-of-month> (ordinal)the <day-of-month> (ordinal or number) of <named-month>",
                                     -4.137829674161839),
                                    ("time-of-day (latent)<day-of-month> (ordinal)",
                                     -4.543294782270004),
                                    ("<day-of-month> (ordinal)time-of-day (latent)",
                                     -3.8501476017100584),
                                    ("<named-month>|<named-day> <day-of-month> (ordinal)year (latent)",
                                     -4.543294782270004),
                                    ("hh:mmtime-of-day (latent)", -3.6270040503958487),
                                    ("hh:mm<time-of-day> am|pm", -4.137829674161839),
                                    ("minuteminute", -3.6270040503958487),
                                    ("time-of-day (latent)time-of-day (latent)",
                                     -3.6270040503958487),
                                    ("hh:mmhh:mm", -4.543294782270004),
                                    ("dayday", -2.0583881324820035),
                                    ("the <day-of-month> (ordinal)the <day-of-month> (number)",
                                     -4.137829674161839),
                                    ("the <day-of-month> (number)the <day-of-month> (number)",
                                     -4.543294782270004),
                                    ("time-of-day (latent)the <day-of-month> (ordinal or number) of <named-month>",
                                     -4.137829674161839),
                                    ("hourhour", -3.4446824936018943),
                                    ("time-of-day (latent)the <day-of-month> (ordinal)",
                                     -4.543294782270004),
                                    ("dayyear", -4.543294782270004),
                                    ("the <day-of-month> (ordinal)time-of-day (latent)",
                                     -4.543294782270004),
                                    ("the <day-of-month> (number)time-of-day (latent)",
                                     -4.543294782270004),
                                    ("minutehour", -3.4446824936018943),
                                    ("hh:mmintersect", -4.137829674161839),
                                    ("time-of-day (latent)<day-of-month> (ordinal or number) of <month>",
                                     -4.137829674161839),
                                    ("<named-month>|<named-day> <day-of-month> (ordinal)July",
                                     -4.543294782270004),
                                    ("<day-of-month> (ordinal)<day-of-month> (ordinal or number) <named-month>",
                                     -4.543294782270004),
                                    ("the <day-of-month> (number)the <day-of-month> (ordinal or number) of <month>",
                                     -4.137829674161839),
                                    ("the <day-of-month> (ordinal)the <day-of-month> (ordinal or number) of <month>",
                                     -4.137829674161839),
                                    ("<day-of-month> (ordinal)<day-of-month> (ordinal or number) of <month>",
                                     -4.137829674161839),
                                    ("time-of-day (latent)<time-of-day> am|pm", -4.543294782270004),
                                    ("time-of-day (latent)<day-of-month> (ordinal or number) <named-month>",
                                     -4.543294782270004),
                                    ("time-of-day (latent)the <day-of-month> (number)",
                                     -4.137829674161839),
                                    ("the <day-of-month> (ordinal)<day-of-month> (ordinal or number) <named-month>",
                                     -4.543294782270004),
                                    ("the <day-of-month> (number)<day-of-month> (ordinal or number) <named-month>",
                                     -4.543294782270004),
                                    ("the <day-of-month> (number)the <day-of-month> (ordinal or number) of <named-month>",
                                     -4.137829674161839),
                                    ("the <day-of-month> (ordinal)the <day-of-month> (ordinal or number) of <named-month>",
                                     -4.137829674161839),
                                    ("<day-of-month> (ordinal)<day-of-month> (ordinal or number) of <named-month>",
                                     -4.137829674161839),
                                    ("time-of-day (latent)the <day-of-month> (ordinal or number) of <month>",
                                     -4.137829674161839),
                                    ("time-of-day (latent)<day-of-month> (ordinal or number) of <named-month>",
                                     -4.137829674161839),
                                    ("<day-of-month> (ordinal)the <day-of-month> (number)",
                                     -4.543294782270004),
                                    ("<named-month> <day-of-month> (non ordinal)time-of-day (latent)",
                                     -4.543294782270004)],
                               n = 61}}),
       ("Saturday",
        Classifier{okData =
                     ClassData{prior = -0.11778303565638351,
                               unseen = -2.3025850929940455,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 8},
                   koData =
                     ClassData{prior = -2.1972245773362196,
                               unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1}}),
       ("the <cycle> of <time>",
        Classifier{okData =
                     ClassData{prior = -0.45198512374305727,
                               unseen = -3.295836866004329,
                               likelihoods =
                                 HashMap.fromList
                                   [("yearyear", -2.5649493574615367),
                                    ("monthmonth", -2.5649493574615367),
                                    ("week (grain)<named-month>|<named-day> <day-of-month> (ordinal)",
                                     -2.159484249353372),
                                    ("weekmonth", -2.5649493574615367),
                                    ("week (grain)October", -2.5649493574615367),
                                    ("month (grain)March", -2.5649493574615367),
                                    ("year (grain)year (latent)", -2.5649493574615367),
                                    ("week (grain)<named-month> <day-of-month> (non ordinal)",
                                     -2.159484249353372),
                                    ("weekday", -1.6486586255873816)],
                               n = 7},
                   koData =
                     ClassData{prior = -1.0116009116784799, unseen = -3.044522437723423,
                               likelihoods =
                                 HashMap.fromList
                                   [("second (grain) March", -2.3025850929940455),
                                    ("weekmonth", -2.3025850929940455),
                                    ("week (grain)October", -2.3025850929940455),
                                    ("secondmonth", -1.6094379124341003),
                                    ("second (grain) this|last|next <cycle>", -1.8971199848858813)],
                               n = 4}}),
       ("number.number hours",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.6094379124341003,
                               likelihoods =
                                 HashMap.fromList
                                   [("hour (grain)", -0.6931471805599453),
                                    ("hour", -0.6931471805599453)],
                               n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("from <time-of-day> - <time-of-day> (interval)",
        Classifier{okData =
                     ClassData{prior = -0.7308875085427924,
                               unseen = -3.6888794541139363,
                               likelihoods =
                                 HashMap.fromList
                                   [("hh:mmtime-of-day (latent)", -2.5649493574615367),
                                    ("minuteminute", -1.8718021769015913),
                                    ("hh:mmhh:mm", -1.8718021769015913),
                                    ("<time-of-day> am|pmtime-of-day (latent)",
                                     -2.5649493574615367),
                                    ("hourhour", -2.5649493574615367),
                                    ("hourminute", -2.5649493574615367),
                                    ("minutehour", -2.0541237336955462),
                                    ("time-of-day (latent)<time-of-day> sharp|exactly",
                                     -2.9704144655697013),
                                    ("time-of-day (latent)hh:mm", -2.9704144655697013),
                                    ("<time-of-day> am|pm<time-of-day> am|pm",
                                     -2.5649493574615367)],
                               n = 13},
                   koData =
                     ClassData{prior = -0.6567795363890705,
                               unseen = -3.7376696182833684,
                               likelihoods =
                                 HashMap.fromList
                                   [("hh:mmtime-of-day (latent)", -1.9218125974762528),
                                    ("hh:mm<time-of-day> am|pm", -2.6149597780361984),
                                    ("minuteminute", -2.6149597780361984),
                                    ("time-of-day (latent)time-of-day (latent)",
                                     -1.9218125974762528),
                                    ("hh:mmhh:mm", -3.0204248861443626),
                                    ("hourhour", -1.7676619176489945),
                                    ("minutehour", -1.7676619176489945),
                                    ("time-of-day (latent)<time-of-day> am|pm",
                                     -3.0204248861443626)],
                               n = 14}}),
       ("integer 21..99",
        Classifier{okData =
                     ClassData{prior = -0.916290731874155, unseen = -1.6094379124341003,
                               likelihoods =
                                 HashMap.fromList
                                   [("integer (20..90)integer (0..19)", -0.2876820724517809)],
                               n = 2},
                   koData =
                     ClassData{prior = -0.5108256237659907, unseen = -1.791759469228055,
                               likelihoods =
                                 HashMap.fromList
                                   [("integer (numeric)integer (numeric)", -0.2231435513142097)],
                               n = 3}}),
       ("Global Youth Service Day",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Tisha B'Av",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("yyyy-mm-dd",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.791759469228055,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 4},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("year (latent)",
        Classifier{okData =
                     ClassData{prior = -0.13140146636669767,
                               unseen = -5.480638923341991,
                               likelihoods =
                                 HashMap.fromList
                                   [("integer (numeric)", -2.5425098365810056e-2),
                                    ("intersect 2 numbers", -4.09016919081162)],
                               n = 235},
                   koData =
                     ClassData{prior = -2.0944794190443763,
                               unseen = -3.6375861597263857,
                               likelihoods =
                                 HashMap.fromList
                                   [("integer (numeric)", -1.5314763709643884),
                                    ("negative numbers", -0.43286408229627876),
                                    ("compose by multiplication", -2.2246235515243336)],
                               n = 33}}),
       ("<time> for <duration>",
        Classifier{okData =
                     ClassData{prior = -0.3364722366212129,
                               unseen = -2.9444389791664407,
                               likelihoods =
                                 HashMap.fromList
                                   [("minuteminute", -2.1972245773362196),
                                    ("<time-of-day> am|pm<integer> <unit-of-duration>",
                                     -1.791759469228055),
                                    ("dayday", -1.791759469228055),
                                    ("hourminute", -1.791759469228055),
                                    ("hhhmm<integer> <unit-of-duration>", -2.1972245773362196),
                                    ("<day-of-month> (ordinal or number) <named-month><integer> <unit-of-duration>",
                                     -1.791759469228055)],
                               n = 5},
                   koData =
                     ClassData{prior = -1.252762968495368, unseen = -2.5649493574615367,
                               likelihoods =
                                 HashMap.fromList
                                   [("monthday", -1.3862943611198906),
                                    ("December<integer> <unit-of-duration>", -1.3862943611198906)],
                               n = 2}}),
       ("hhhmm",
        Classifier{okData =
                     ClassData{prior = -2.247285585205863e-2,
                               unseen = -3.828641396489095,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 44},
                   koData =
                     ClassData{prior = -3.8066624897703196,
                               unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1}}),
       ("Mahavir Jayanti",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Monday",
        Classifier{okData =
                     ClassData{prior = -0.21357410029805904,
                               unseen = -3.1354942159291497,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 21},
                   koData =
                     ClassData{prior = -1.6486586255873816,
                               unseen = -1.9459101490553135,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 5}}),
       ("dd/mm/yyyy",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("yesterday",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.9459101490553135,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 5},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<ordinal> quarter <year>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.9459101490553135,
                               likelihoods =
                                 HashMap.fromList
                                   [("ordinal (digits)quarter (grain)year (latent)",
                                     -0.6931471805599453),
                                    ("quarteryear", -0.6931471805599453)],
                               n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("the last|past|next <cycle>",
        Classifier{okData =
                     ClassData{prior = -infinity, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [], n = 0},
                   koData =
                     ClassData{prior = 0.0, unseen = -1.6094379124341003,
                               likelihoods =
                                 HashMap.fromList
                                   [("month (grain)", -0.6931471805599453),
                                    ("month", -0.6931471805599453)],
                               n = 1}}),
       ("Dayananda Saraswati Jayanti",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("hh:mm:ss",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.9459101490553135,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 5},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Hanukkah",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.6094379124341003,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 3},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("first|second|third|fourth|fifth <day-of-week> of <time>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -3.6635616461296463,
                               likelihoods =
                                 HashMap.fromList
                                   [("daymonth", -0.8649974374866046),
                                    ("ordinals (first..twentieth,thirtieth,...)Tuesdaythis|last|next <cycle>",
                                     -2.9444389791664407),
                                    ("ordinals (first..twentieth,thirtieth,...)TuesdaySeptember",
                                     -2.9444389791664407),
                                    ("ordinals (first..twentieth,thirtieth,...)Tuesdayintersect",
                                     -2.9444389791664407),
                                    ("ordinals (first..twentieth,thirtieth,...)WednesdayOctober",
                                     -2.538973871058276),
                                    ("ordinals (first..twentieth,thirtieth,...)Wednesdayintersect",
                                     -2.538973871058276),
                                    ("ordinals (first..twentieth,thirtieth,...)Mondaythis|last|next <cycle>",
                                     -1.6916760106710724),
                                    ("ordinals (first..twentieth,thirtieth,...)TuesdayOctober",
                                     -2.538973871058276)],
                               n = 15},
                   koData =
                     ClassData{prior = -infinity, unseen = -2.1972245773362196,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<hour-of-day> <integer>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -3.295836866004329,
                               likelihoods =
                                 HashMap.fromList
                                   [("at <time-of-day>integer (20..90)", -2.5649493574615367),
                                    ("time-of-day (latent)integer (numeric)", -2.159484249353372),
                                    ("time-of-day (latent)integer 21..99", -2.159484249353372),
                                    ("hour", -0.8602012652231115),
                                    ("at <time-of-day>integer (numeric)", -2.159484249353372),
                                    ("time-of-day (latent)integer (20..90)", -1.8718021769015913)],
                               n = 10},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.9459101490553135,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Rama Navami",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("nth <time> of <time>",
        Classifier{okData =
                     ClassData{prior = -0.2876820724517809, unseen = -3.258096538021482,
                               likelihoods =
                                 HashMap.fromList
                                   [("daymonth", -0.916290731874155),
                                    ("ordinals (first..twentieth,thirtieth,...)Tuesdayintersect",
                                     -2.5257286443082556),
                                    ("ordinals (first..twentieth,thirtieth,...)Wednesdayintersect",
                                     -2.120263536200091),
                                    ("ordinals (first..twentieth,thirtieth,...)Mondaythis|last|next <cycle>",
                                     -1.6094379124341003),
                                    ("ordinals (first..twentieth,thirtieth,...)TuesdayOctober",
                                     -2.120263536200091)],
                               n = 9},
                   koData =
                     ClassData{prior = -1.3862943611198906, unseen = -2.639057329615259,
                               likelihoods =
                                 HashMap.fromList
                                   [("daymonth", -1.1786549963416462),
                                    ("ordinals (first..twentieth,thirtieth,...)TuesdaySeptember",
                                     -1.8718021769015913),
                                    ("ordinals (first..twentieth,thirtieth,...)WednesdayOctober",
                                     -1.466337068793427)],
                               n = 3}}),
       ("after <time>",
        Classifier{okData =
                     ClassData{prior = -1.3862943611198906,
                               unseen = -3.0910424533583156,
                               likelihoods =
                                 HashMap.fromList
                                   [("intersect", -2.3513752571634776),
                                    ("year (latent)", -2.3513752571634776),
                                    ("time-of-day (latent)", -2.3513752571634776),
                                    ("year", -2.3513752571634776),
                                    ("<time-of-day> am|pm", -2.3513752571634776),
                                    ("hour", -1.9459101490553135), ("month", -2.3513752571634776)],
                               n = 4},
                   koData =
                     ClassData{prior = -0.2876820724517809,
                               unseen = -3.6375861597263857,
                               likelihoods =
                                 HashMap.fromList
                                   [("December", -2.917770732084279),
                                    ("intersect", -2.512305623976115),
                                    ("tomorrow", -2.512305623976115), ("day", -1.6650077635889111),
                                    ("time-of-day (latent)", -2.512305623976115),
                                    ("<time-of-day> am|pm", -2.917770732084279),
                                    ("Easter Sunday", -2.917770732084279),
                                    ("Christmas", -2.512305623976115),
                                    ("hour", -1.8191584434161694), ("month", -2.917770732084279),
                                    ("part of days", -2.917770732084279)],
                               n = 12}}),
       ("<hour-of-day> quarter",
        Classifier{okData =
                     ClassData{prior = -infinity, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [], n = 0},
                   koData =
                     ClassData{prior = 0.0, unseen = -1.9459101490553135,
                               likelihoods =
                                 HashMap.fromList
                                   [("time-of-day (latent)", -0.6931471805599453),
                                    ("hour", -0.6931471805599453)],
                               n = 2}}),
       ("half a <time-grain>",
        Classifier{okData =
                     ClassData{prior = -0.1823215567939546,
                               unseen = -2.5649493574615367,
                               likelihoods =
                                 HashMap.fromList
                                   [("hour (grain)", -0.6931471805599453),
                                    ("hour", -0.6931471805599453)],
                               n = 5},
                   koData =
                     ClassData{prior = -1.791759469228055, unseen = -1.6094379124341003,
                               likelihoods =
                                 HashMap.fromList
                                   [("hour (grain)", -0.6931471805599453),
                                    ("hour", -0.6931471805599453)],
                               n = 1}}),
       ("King's Day",
        Classifier{okData =
                     ClassData{prior = -0.11778303565638351,
                               unseen = -2.3025850929940455,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 8},
                   koData =
                     ClassData{prior = -2.1972245773362196,
                               unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1}}),
       ("Valentine's Day",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("from the <day-of-month> (ordinal or number) to the <day-of-month> (ordinal or number) of <named-month> (interval)",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -3.4657359027997265,
                               likelihoods =
                                 HashMap.fromList
                                   [("ordinal (digits)ordinal (digits)July", -1.824549292051046),
                                    ("integer (numeric)integer (numeric)July", -2.0476928433652555),
                                    ("integer (numeric)ordinal (digits)July", -2.0476928433652555),
                                    ("ordinal (digits)integer (numeric)July", -2.0476928433652555),
                                    ("month", -0.7949298748698876)],
                               n = 13},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.791759469228055,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("April",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.1972245773362196,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 7},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("from the <day-of-month> (ordinal or number) of <named-month> (interval) to the <day-of-month> (ordinal or number)",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.4849066497880004,
                               likelihoods =
                                 HashMap.fromList
                                   [("ordinal (digits)Julyordinal (digits)", -1.2992829841302609),
                                    ("ordinal (digits)Julyinteger (numeric)", -1.2992829841302609),
                                    ("month", -0.7884573603642702)],
                               n = 4},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Maha Saptami",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("end of month",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.4849066497880004,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 10},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("week (grain)",
        Classifier{okData =
                     ClassData{prior = -3.4289073478632075e-2,
                               unseen = -4.477336814478207,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 86},
                   koData =
                     ClassData{prior = -3.39002408106403, unseen = -1.6094379124341003,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 3}}),
       ("<part-of-day> of <time>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -3.2188758248682006,
                               likelihoods =
                                 HashMap.fromList
                                   [("part of daysintersect", -2.4849066497880004),
                                    ("hourday", -0.9808292530117262),
                                    ("part of daysthe <day-of-month> (ordinal)",
                                     -2.4849066497880004),
                                    ("part of daysthe <day-of-month> (ordinal or number) of <month>",
                                     -2.4849066497880004),
                                    ("part of daysthe <day-of-month> (number)",
                                     -2.4849066497880004),
                                    ("part of daysthis <time>", -2.4849066497880004),
                                    ("part of daysthe <day-of-month> (ordinal or number) of <named-month>",
                                     -2.4849066497880004),
                                    ("part of daysChristmas", -2.0794415416798357)],
                               n = 8},
                   koData =
                     ClassData{prior = -infinity, unseen = -2.1972245773362196,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("from <time> for <duration>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.1972245773362196,
                               likelihoods =
                                 HashMap.fromList
                                   [("<time-of-day> am|pm<integer> <unit-of-duration>",
                                     -1.3862943611198906),
                                    ("dayday", -1.3862943611198906),
                                    ("hourminute", -1.3862943611198906),
                                    ("<day-of-month> (ordinal or number) <named-month><integer> <unit-of-duration>",
                                     -1.3862943611198906)],
                               n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.6094379124341003,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("six thirty six a.m.",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.9459101490553135,
                               likelihoods =
                                 HashMap.fromList
                                   [("integer (numeric)", -1.0986122886681098),
                                    ("one twenty two", -1.0986122886681098),
                                    ("one eleven", -1.0986122886681098)],
                               n = 3},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<integer> <day-of-week> from <time>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.995732273553991,
                               likelihoods =
                                 HashMap.fromList
                                   [("integer (numeric)Fridaynow", -2.2512917986064953),
                                    ("integer (0..19)Tuesdaynow", -2.2512917986064953),
                                    ("integer (0..19)Sundaynow", -2.2512917986064953),
                                    ("integer (0..19)Fridaynow", -2.2512917986064953),
                                    ("daynograin", -0.9985288301111273),
                                    ("integer (numeric)Sundaynow", -2.2512917986064953),
                                    ("integer (numeric)Tuesdaynow", -2.2512917986064953)],
                               n = 6},
                   koData =
                     ClassData{prior = -infinity, unseen = -2.0794415416798357,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Shrove Tuesday",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("now",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.3978952727983707,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 9},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<day-of-month> (ordinal or number) of <named-month>",
        Classifier{okData =
                     ClassData{prior = -0.6931471805599453,
                               unseen = -3.7376696182833684,
                               likelihoods =
                                 HashMap.fromList
                                   [("ordinal (digits)July", -3.0204248861443626),
                                    ("ordinal (digits)January", -3.0204248861443626),
                                    ("ordinals (first..twentieth,thirtieth,...)March",
                                     -1.7676619176489945),
                                    ("ordinal (digits)February", -2.6149597780361984),
                                    ("integer (numeric)February", -2.6149597780361984),
                                    ("month", -0.8803587226480918),
                                    ("ordinal (digits)March", -2.6149597780361984),
                                    ("ordinals (first..twentieth,thirtieth,...)February",
                                     -3.0204248861443626),
                                    ("integer (numeric)July", -3.0204248861443626)],
                               n = 16},
                   koData =
                     ClassData{prior = -0.6931471805599453,
                               unseen = -3.7376696182833684,
                               likelihoods =
                                 HashMap.fromList
                                   [("ordinal (digits)July", -1.410986973710262),
                                    ("ordinal (digits)February", -3.0204248861443626),
                                    ("month", -0.8803587226480918),
                                    ("integer (numeric)July", -1.7676619176489945)],
                               n = 16}}),
       ("this <part-of-day>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -3.4339872044851463,
                               likelihoods =
                                 HashMap.fromList
                                   [("hour", -0.6931471805599453),
                                    ("part of days", -0.6931471805599453)],
                               n = 14},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Kaanum Pongal",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Yom Kippur",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<integer> minutes to|till|before <hour-of-day>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.6094379124341003,
                               likelihoods =
                                 HashMap.fromList
                                   [("integer (numeric)minute (grain)time-of-day (latent)",
                                     -0.6931471805599453),
                                    ("minutehour", -0.6931471805599453)],
                               n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("the <day-of-month> (ordinal or number) of <month>",
        Classifier{okData =
                     ClassData{prior = -0.42488319396526597,
                               unseen = -3.8066624897703196,
                               likelihoods =
                                 HashMap.fromList
                                   [("ordinal (digits)July", -3.0910424533583156),
                                    ("ordinal (digits)January", -3.0910424533583156),
                                    ("ordinals (first..twentieth,thirtieth,...)March",
                                     -2.174751721484161),
                                    ("ordinal (digits)this|last|next <cycle>", -2.6855773452501515),
                                    ("ordinal (digits)intersect", -2.6855773452501515),
                                    ("ordinal (digits)February", -2.3978952727983707),
                                    ("month", -0.8938178760220964),
                                    ("ordinals (first..twentieth,thirtieth,...)this|last|next <cycle>",
                                     -2.6855773452501515),
                                    ("ordinal (digits)March", -2.6855773452501515)],
                               n = 17},
                   koData =
                     ClassData{prior = -1.0608719606852628, unseen = -3.367295829986474,
                               likelihoods =
                                 HashMap.fromList
                                   [("ordinal (digits)July", -1.540445040947149),
                                    ("month", -1.0296194171811581),
                                    ("integer (numeric)July", -1.7227665977411035)],
                               n = 9}}),
       ("Rabindra Jayanti",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.0794415416798357,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 6},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("powers of tens",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.6094379124341003,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 3},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Pargat Diwas",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.0794415416798357,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 6},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Friday",
        Classifier{okData =
                     ClassData{prior = -0.3184537311185346, unseen = -2.890371757896165,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 16},
                   koData =
                     ClassData{prior = -1.2992829841302609,
                               unseen = -2.0794415416798357,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 6}}),
       ("in|during the <part-of-day>",
        Classifier{okData =
                     ClassData{prior = -6.899287148695143e-2,
                               unseen = -3.4657359027997265,
                               likelihoods =
                                 HashMap.fromList
                                   [("early morning", -2.740840023925201),
                                    ("hour", -0.7259370033829361),
                                    ("part of days", -0.7949298748698876)],
                               n = 14},
                   koData =
                     ClassData{prior = -2.70805020110221, unseen = -1.791759469228055,
                               likelihoods =
                                 HashMap.fromList
                                   [("hour", -0.916290731874155),
                                    ("part of days", -0.916290731874155)],
                               n = 1}}),
       ("St Patrick's Day",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.791759469228055,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 4},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("tomorrow",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -3.1780538303479458,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 22},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("hh(:mm) - <time-of-day> am|pm",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -3.1354942159291497,
                               likelihoods =
                                 HashMap.fromList
                                   [("time-of-day (latent)", -1.0116009116784799),
                                    ("hh:mm", -1.9924301646902063), ("hour", -1.0116009116784799),
                                    ("minute", -1.9924301646902063)],
                               n = 9},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.6094379124341003,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("this|last|next <cycle>",
        Classifier{okData =
                     ClassData{prior = -0.29815337231907635,
                               unseen = -5.303304908059076,
                               likelihoods =
                                 HashMap.fromList
                                   [("week", -1.3093333199837622),
                                    ("month (grain)", -2.0794415416798357),
                                    ("year (grain)", -2.5902671654458267),
                                    ("week (grain)", -1.3093333199837622),
                                    ("quarter", -3.6888794541139363), ("year", -2.5902671654458267),
                                    ("month", -2.0794415416798357),
                                    ("quarter (grain)", -3.6888794541139363)],
                               n = 95},
                   koData =
                     ClassData{prior = -1.3555227024531369, unseen = -4.343805421853684,
                               likelihoods =
                                 HashMap.fromList
                                   [("week", -1.6916760106710724),
                                    ("month (grain)", -2.7212954278522306),
                                    ("year (grain)", -2.7212954278522306),
                                    ("week (grain)", -1.6916760106710724),
                                    ("day", -2.133508762950112), ("quarter", -2.7212954278522306),
                                    ("year", -2.7212954278522306), ("month", -2.7212954278522306),
                                    ("quarter (grain)", -2.7212954278522306),
                                    ("day (grain)", -2.133508762950112)],
                               n = 33}}),
       ("Simchat Torah",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("New Year's Eve",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("the <ordinal> <cycle> of <time>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.6094379124341003,
                               likelihoods =
                                 HashMap.fromList
                                   [("ordinal (digits)quarter (grain)year (latent)",
                                     -0.6931471805599453),
                                    ("quarteryear", -0.6931471805599453)],
                               n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Raksha Bandhan",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Ashura",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Ratha-Yatra",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Palm Sunday",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Eid al-Adha",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.890371757896165,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 16},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("by <time>",
        Classifier{okData =
                     ClassData{prior = -0.2231435513142097, unseen = -2.833213344056216,
                               likelihoods =
                                 HashMap.fromList
                                   [("noon|midnight|EOD|end of day", -2.0794415416798357),
                                    ("time-of-day (latent)", -2.0794415416798357),
                                    ("<time-of-day> am|pm", -2.0794415416798357),
                                    ("hh:mm", -2.0794415416798357), ("hour", -1.6739764335716716),
                                    ("minute", -1.6739764335716716)],
                               n = 4},
                   koData =
                     ClassData{prior = -1.6094379124341003,
                               unseen = -2.3978952727983707,
                               likelihoods =
                                 HashMap.fromList
                                   [("at the beginning|end of <year>", -1.6094379124341003),
                                    ("month", -1.6094379124341003)],
                               n = 1}}),
       ("through <time>",
        Classifier{okData =
                     ClassData{prior = -0.8472978603872037, unseen = -2.890371757896165,
                               likelihoods =
                                 HashMap.fromList
                                   [("year (latent)", -2.1400661634962708),
                                    ("year", -2.1400661634962708),
                                    ("<time-of-day> am|pm", -2.1400661634962708),
                                    ("hh:mm", -2.1400661634962708),
                                    ("minute", -1.7346010553881064)],
                               n = 3},
                   koData =
                     ClassData{prior = -0.5596157879354228, unseen = -2.995732273553991,
                               likelihoods =
                                 HashMap.fromList
                                   [("July", -2.2512917986064953), ("day", -2.2512917986064953),
                                    ("time-of-day (latent)", -1.845826690498331),
                                    ("hour", -1.845826690498331), ("month", -2.2512917986064953),
                                    ("<named-month> <day-of-month> (non ordinal)",
                                     -2.2512917986064953)],
                               n = 4}}),
       ("the <day-of-month> (ordinal)",
        Classifier{okData =
                     ClassData{prior = -0.15415067982725836,
                               unseen = -2.639057329615259,
                               likelihoods = HashMap.fromList [("ordinal (digits)", 0.0)],
                               n = 12},
                   koData =
                     ClassData{prior = -1.9459101490553135,
                               unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("ordinal (digits)", 0.0)],
                               n = 2}}),
       ("last weekend of <named-month>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.639057329615259,
                               likelihoods =
                                 HashMap.fromList
                                   [("October", -0.9555114450274363), ("July", -1.8718021769015913),
                                    ("month", -0.7731898882334817)],
                               n = 5},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("the <day-of-month> (number)",
        Classifier{okData =
                     ClassData{prior = -0.6359887667199967,
                               unseen = -2.3978952727983707,
                               likelihoods = HashMap.fromList [("integer (numeric)", 0.0)],
                               n = 9},
                   koData =
                     ClassData{prior = -0.7537718023763802,
                               unseen = -2.3025850929940455,
                               likelihoods = HashMap.fromList [("integer (numeric)", 0.0)],
                               n = 8}}),
       ("Lag BaOmer",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("fractional number",
        Classifier{okData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0},
                   koData =
                     ClassData{prior = 0.0, unseen = -2.772588722239781,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 14}}),
       ("Guru Ravidass Jayanti",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.0794415416798357,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 6},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Sunday",
        Classifier{okData =
                     ClassData{prior = -0.40546510810816444,
                               unseen = -2.4849066497880004,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 10},
                   koData =
                     ClassData{prior = -1.0986122886681098,
                               unseen = -1.9459101490553135,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 5}}),
       ("June",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -3.4965075614664802,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 31},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Chinese New Year",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.0794415416798357,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 6},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("February",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.833213344056216,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 15},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("minute (grain)",
        Classifier{okData =
                     ClassData{prior = -7.696104113612832e-2,
                               unseen = -3.295836866004329,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 25},
                   koData =
                     ClassData{prior = -2.6026896854443837,
                               unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2}}),
       ("last|this|next <season>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.0794415416798357,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 6},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("the <ordinal> quarter",
        Classifier{okData =
                     ClassData{prior = -0.6931471805599453,
                               unseen = -1.6094379124341003,
                               likelihoods =
                                 HashMap.fromList
                                   [("ordinal (digits)quarter (grain)", -0.6931471805599453),
                                    ("quarter", -0.6931471805599453)],
                               n = 1},
                   koData =
                     ClassData{prior = -0.6931471805599453,
                               unseen = -1.6094379124341003,
                               likelihoods =
                                 HashMap.fromList
                                   [("ordinal (digits)quarter (grain)", -0.6931471805599453),
                                    ("quarter", -0.6931471805599453)],
                               n = 1}}),
       ("Orthodox Good Friday",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("time-of-day (latent)",
        Classifier{okData =
                     ClassData{prior = -0.48489224173948614,
                               unseen = -4.852030263919617,
                               likelihoods =
                                 HashMap.fromList
                                   [("integer (numeric)", -9.925495809534127e-2),
                                    ("integer (0..19)", -2.359280436670591)],
                               n = 125},
                   koData =
                     ClassData{prior = -0.9564971523521957, unseen = -4.394449154672439,
                               likelihoods =
                                 HashMap.fromList
                                   [("integer (numeric)", -5.129329438755058e-2),
                                    ("integer (0..19)", -2.995732273553991)],
                               n = 78}}),
       ("beginning of year",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.791759469228055,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 4},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("last <day-of-week> of <time>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.772588722239781,
                               likelihoods =
                                 HashMap.fromList
                                   [("daymonth", -0.916290731874155),
                                    ("SundayMarch", -2.0149030205422647),
                                    ("MondayMarch", -2.0149030205422647),
                                    ("FridayOctober", -1.6094379124341003),
                                    ("Sundayintersect", -2.0149030205422647)],
                               n = 5},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.791759469228055,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<integer> <unit-of-duration>",
        Classifier{okData =
                     ClassData{prior = -0.6164133863285559, unseen = -5.308267697401205,
                               likelihoods =
                                 HashMap.fromList
                                   [("week", -2.6642475784438173),
                                    ("integer (0..19)year (grain)", -3.22386336637924),
                                    ("integer (numeric)day (grain)", -2.6642475784438173),
                                    ("integer (0..19)second (grain) ", -3.917010546939185),
                                    ("integer (0..19)hour (grain)", -3.1060803307228566),
                                    ("second", -3.5115454388310208),
                                    ("integer (numeric)second (grain) ", -4.204692619390966),
                                    ("integer (numeric)year (grain)", -3.3573947590037623),
                                    ("day", -2.412933150162911), ("year", -2.6642475784438173),
                                    ("integer (numeric)week (grain)", -3.22386336637924),
                                    ("integer (0..19)month (grain)", -3.6938669956249752),
                                    ("integer (20..90)minute (grain)", -4.61015772749913),
                                    ("hour", -2.738355550597539), ("month", -3.3573947590037623),
                                    ("integer (numeric)minute (grain)", -2.5952547069568657),
                                    ("integer (0..19)minute (grain)", -3.22386336637924),
                                    ("integer (numeric)month (grain)", -4.204692619390966),
                                    ("minute", -2.167810692129926),
                                    ("integer (numeric)hour (grain)", -3.6938669956249752),
                                    ("integer (0..19)day (grain)", -3.6938669956249752),
                                    ("integer (0..19)week (grain)", -3.3573947590037623)],
                               n = 88},
                   koData =
                     ClassData{prior = -0.7762620872704519, unseen = -5.170483995038151,
                               likelihoods =
                                 HashMap.fromList
                                   [("week", -3.5553480614894135),
                                    ("integer (0..19)year (grain)", -4.0661736852554045),
                                    ("integer (numeric)day (grain)", -4.0661736852554045),
                                    ("integer (numeric)quarter (grain)", -4.0661736852554045),
                                    ("integer (numeric)year (grain)", -4.0661736852554045),
                                    ("day", -3.5553480614894135), ("quarter", -3.5553480614894135),
                                    ("year", -3.5553480614894135),
                                    ("integer (numeric)week (grain)", -4.0661736852554045),
                                    ("integer (0..19)month (grain)", -4.0661736852554045),
                                    ("hour", -1.157452788691043), ("month", -3.5553480614894135),
                                    ("integer (numeric)minute (grain)", -4.471638793363569),
                                    ("integer (numeric)month (grain)", -4.0661736852554045),
                                    ("minute", -4.471638793363569),
                                    ("integer (numeric)hour (grain)", -1.157452788691043),
                                    ("integer (0..19)day (grain)", -4.0661736852554045),
                                    ("integer (0..19)week (grain)", -4.0661736852554045),
                                    ("integer (0..19)quarter (grain)", -4.0661736852554045)],
                               n = 75}}),
       ("from the <day-of-month> (ordinal or number) to the <day-of-month> (ordinal or number) <named-month> (interval)",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.772588722239781,
                               likelihoods =
                                 HashMap.fromList
                                   [("ordinal (digits)ordinal (digits)July", -1.3217558399823195),
                                    ("integer (numeric)integer (numeric)July", -1.3217558399823195),
                                    ("month", -0.7621400520468967)],
                               n = 6},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Guru Gobind Singh Jayanti",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.791759469228055,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 4},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("hhmm (latent)",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.9459101490553135,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 5},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Krishna Janmashtami",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<time-of-day> am|pm",
        Classifier{okData =
                     ClassData{prior = -0.2480168160593495, unseen = -5.66988092298052,
                               likelihoods =
                                 HashMap.fromList
                                   [("integer after|past <hour-of-day>", -4.2801323269925415),
                                    ("at <time-of-day>", -2.370589822108103),
                                    ("<time-of-day> o'clock", -4.567814399444322),
                                    ("half after|past <hour-of-day>", -4.567814399444322),
                                    ("second", -4.567814399444322),
                                    ("hh:mm:ss", -4.567814399444322),
                                    ("<hour-of-day> <integer>", -4.056988775678332),
                                    ("<integer> minutes to|till|before <hour-of-day>",
                                     -4.973279507552487),
                                    ("time-of-day (latent)", -1.6774426415481578),
                                    ("hhmm (latent)", -4.567814399444322),
                                    ("hh:mm", -2.370589822108103),
                                    ("quarter after|past <hour-of-day>", -3.4692021107762128),
                                    ("until <time>", -4.567814399444322),
                                    ("about|exactly <time-of-day>", -4.567814399444322),
                                    ("since <time>", -4.973279507552487),
                                    ("hour", -1.2844000534385505),
                                    ("<time-of-day> sharp|exactly", -4.973279507552487),
                                    ("minute", -1.6590935028799612)],
                               n = 135},
                   koData =
                     ClassData{prior = -1.5157054347713932, unseen = -4.564348191467836,
                               likelihoods =
                                 HashMap.fromList
                                   [("<integer> to|till|before <hour-of-day>", -3.1675825304806504),
                                    ("at <time-of-day>", -3.455264602932431),
                                    ("time-of-day (latent)", -1.1198896871153945),
                                    ("hh:mm", -3.455264602932431),
                                    ("until <time>", -3.8607297110405954),
                                    ("hour", -1.0275163669843794), ("minute", -2.762117422372486)],
                               n = 38}}),
       ("Yom Ha'atzmaut",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("on <day>",
        Classifier{okData =
                     ClassData{prior = -0.3794896217049037,
                               unseen = -4.1588830833596715,
                               likelihoods =
                                 HashMap.fromList
                                   [("Thursday", -1.9459101490553135),
                                    ("absorption of , after named day", -2.7568403652716422),
                                    ("the <day-of-month> (ordinal or number) of <named-month>",
                                     -3.4499875458315876),
                                    ("intersect", -2.7568403652716422),
                                    ("Saturday", -2.7568403652716422),
                                    ("the <day-of-month> (ordinal or number) of <month>",
                                     -2.7568403652716422),
                                    ("Friday", -3.044522437723423), ("day", -0.8472978603872037),
                                    ("intersect by \",\", \"of\", \"from\", \"'s\"",
                                     -2.7568403652716422)],
                               n = 26},
                   koData =
                     ClassData{prior = -1.1526795099383855, unseen = -3.58351893845611,
                               likelihoods =
                                 HashMap.fromList
                                   [("the <day-of-month> (ordinal or number) of <named-month>",
                                     -2.8622008809294686),
                                    ("intersect", -2.169053700369523),
                                    ("the <day-of-month> (ordinal or number) of <month>",
                                     -2.8622008809294686),
                                    ("day", -0.9903987040278769),
                                    ("intersect by \",\", \"of\", \"from\", \"'s\"",
                                     -2.169053700369523),
                                    ("<named-month>|<named-day> <day-of-month> (ordinal)",
                                     -2.456735772821304),
                                    ("<named-month> <day-of-month> (non ordinal)",
                                     -2.456735772821304)],
                               n = 12}}),
       ("Thiru Onam",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.6094379124341003,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 3},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Thai Pongal",
        Classifier{okData =
                     ClassData{prior = -0.5596157879354228, unseen = -1.791759469228055,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 4},
                   koData =
                     ClassData{prior = -0.8472978603872037,
                               unseen = -1.6094379124341003,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 3}}),
       ("Vijayadashami",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("part of <named-month>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.1972245773362196,
                               likelihoods =
                                 HashMap.fromList
                                   [("March", -0.6931471805599453), ("month", -0.6931471805599453)],
                               n = 3},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("a <unit-of-duration>",
        Classifier{okData =
                     ClassData{prior = -1.0296194171811581,
                               unseen = -3.5553480614894135,
                               likelihoods =
                                 HashMap.fromList
                                   [("week", -2.1400661634962708),
                                    ("year (grain)", -2.4277482359480516),
                                    ("second", -2.833213344056216),
                                    ("week (grain)", -2.1400661634962708),
                                    ("day", -2.1400661634962708),
                                    ("minute (grain)", -2.833213344056216),
                                    ("year", -2.4277482359480516),
                                    ("second (grain) ", -2.833213344056216),
                                    ("minute", -2.833213344056216),
                                    ("day (grain)", -2.1400661634962708)],
                               n = 10},
                   koData =
                     ClassData{prior = -0.4418327522790392,
                               unseen = -3.9318256327243257,
                               likelihoods =
                                 HashMap.fromList
                                   [("hour (grain)", -2.3025850929940455),
                                    ("quarter", -1.3470736479666092),
                                    ("minute (grain)", -2.8134107167600364),
                                    ("hour", -2.3025850929940455),
                                    ("quarter (grain)", -1.3470736479666092),
                                    ("minute", -2.8134107167600364)],
                               n = 18}}),
       ("at the beginning|end of <year>",
        Classifier{okData =
                     ClassData{prior = -0.10536051565782628,
                               unseen = -3.0910424533583156,
                               likelihoods =
                                 HashMap.fromList
                                   [("year (latent)", -1.0986122886681098),
                                    ("this|last|next <cycle>", -1.6582280766035324),
                                    ("year", -0.7419373447293773)],
                               n = 9},
                   koData =
                     ClassData{prior = -2.3025850929940455, unseen = -1.791759469228055,
                               likelihoods =
                                 HashMap.fromList
                                   [("this|last|next <cycle>", -0.916290731874155),
                                    ("year", -0.916290731874155)],
                               n = 1}}),
       ("Dhanteras",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Tu BiShvat",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Whit Monday",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("hh:mm",
        Classifier{okData =
                     ClassData{prior = -0.13353139262452263,
                               unseen = -4.276666119016055,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 70},
                   koData =
                     ClassData{prior = -2.0794415416798357,
                               unseen = -2.4849066497880004,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 10}}),
       ("Holi",
        Classifier{okData =
                     ClassData{prior = -0.2876820724517809,
                               unseen = -1.6094379124341003,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 3},
                   koData =
                     ClassData{prior = -1.3862943611198906,
                               unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1}}),
       ("upcoming <integer> <cycle>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -4.02535169073515,
                               likelihoods =
                                 HashMap.fromList
                                   [("week", -2.3978952727983707),
                                    ("integer (0..19)year (grain)", -2.908720896564361),
                                    ("integer (numeric)day (grain)", -2.908720896564361),
                                    ("integer (numeric)quarter (grain)", -2.908720896564361),
                                    ("integer (numeric)year (grain)", -2.908720896564361),
                                    ("day", -2.3978952727983707), ("quarter", -2.3978952727983707),
                                    ("year", -2.3978952727983707),
                                    ("integer (numeric)week (grain)", -2.908720896564361),
                                    ("integer (0..19)month (grain)", -2.908720896564361),
                                    ("month", -2.3978952727983707),
                                    ("integer (numeric)month (grain)", -2.908720896564361),
                                    ("integer (0..19)day (grain)", -2.908720896564361),
                                    ("integer (0..19)week (grain)", -2.908720896564361),
                                    ("integer (0..19)quarter (grain)", -2.908720896564361)],
                               n = 20},
                   koData =
                     ClassData{prior = -infinity, unseen = -2.772588722239781,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Rosh Hashanah",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.6094379124341003,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 3},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<integer> <named-day> ago|back",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.9459101490553135,
                               likelihoods =
                                 HashMap.fromList
                                   [("integer (numeric)Thursday", -0.6931471805599453),
                                    ("day", -0.6931471805599453)],
                               n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<integer> upcoming <cycle>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -4.02535169073515,
                               likelihoods =
                                 HashMap.fromList
                                   [("week", -2.3978952727983707),
                                    ("integer (0..19)year (grain)", -2.908720896564361),
                                    ("integer (numeric)day (grain)", -2.908720896564361),
                                    ("integer (numeric)quarter (grain)", -2.908720896564361),
                                    ("integer (numeric)year (grain)", -2.908720896564361),
                                    ("day", -2.3978952727983707), ("quarter", -2.3978952727983707),
                                    ("year", -2.3978952727983707),
                                    ("integer (numeric)week (grain)", -2.908720896564361),
                                    ("integer (0..19)month (grain)", -2.908720896564361),
                                    ("month", -2.3978952727983707),
                                    ("integer (numeric)month (grain)", -2.908720896564361),
                                    ("integer (0..19)day (grain)", -2.908720896564361),
                                    ("integer (0..19)week (grain)", -2.908720896564361),
                                    ("integer (0..19)quarter (grain)", -2.908720896564361)],
                               n = 20},
                   koData =
                     ClassData{prior = -infinity, unseen = -2.772588722239781,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Holika Dahan",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.6094379124341003,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 3},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("quarter of an hour",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.791759469228055,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 4},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("second (grain) ",
        Classifier{okData =
                     ClassData{prior = -0.40546510810816444,
                               unseen = -2.0794415416798357,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 6},
                   koData =
                     ClassData{prior = -1.0986122886681098,
                               unseen = -1.6094379124341003,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 3}}),
       ("ordinals (first..twentieth,thirtieth,...)",
        Classifier{okData =
                     ClassData{prior = -2.5317807984289897e-2,
                               unseen = -3.713572066704308,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 39},
                   koData =
                     ClassData{prior = -3.6888794541139363,
                               unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1}}),
       ("Mawlid",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Jumu'atul-Wida",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.791759469228055,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 4},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<duration> after|before|from|past <time>",
        Classifier{okData =
                     ClassData{prior = -0.7444404749474959, unseen = -4.174387269895637,
                               likelihoods =
                                 HashMap.fromList
                                   [("a <unit-of-duration>now", -3.4657359027997265),
                                    ("<integer> <unit-of-duration>hhhmm", -3.4657359027997265),
                                    ("a <unit-of-duration>Christmas", -3.4657359027997265),
                                    ("minuteminute", -3.4657359027997265),
                                    ("dayday", -3.4657359027997265),
                                    ("<integer> <unit-of-duration>today", -3.4657359027997265),
                                    ("<integer> <unit-of-duration>time-of-day (latent)",
                                     -2.367123614131617),
                                    ("minutehour", -1.7609878105613013),
                                    ("daysecond", -3.4657359027997265),
                                    ("a <unit-of-duration>right now", -3.4657359027997265),
                                    ("minutenograin", -3.4657359027997265),
                                    ("<integer> <unit-of-duration>Christmas", -3.4657359027997265),
                                    ("<integer> <unit-of-duration>Easter Sunday",
                                     -3.4657359027997265),
                                    ("secondnograin", -3.4657359027997265),
                                    ("<integer> <unit-of-duration><time-of-day> am|pm",
                                     -2.5494451709255714),
                                    ("yearday", -2.772588722239781),
                                    ("<integer> <unit-of-duration>noon|midnight|EOD|end of day",
                                     -3.4657359027997265),
                                    ("daynograin", -3.4657359027997265),
                                    ("<integer> <unit-of-duration>now", -3.0602707946915624)],
                               n = 19},
                   koData =
                     ClassData{prior = -0.6443570163905132, unseen = -4.23410650459726,
                               likelihoods =
                                 HashMap.fromList
                                   [("quarterhour", -1.2750687260096663),
                                    ("dayhour", -3.5263605246161616),
                                    ("<integer> <unit-of-duration><day-of-month> (ordinal)",
                                     -3.5263605246161616),
                                    ("a <unit-of-duration><time-of-day> am|pm",
                                     -2.0222831278398874),
                                    ("a <unit-of-duration>time-of-day (latent)",
                                     -2.0222831278398874),
                                    ("dayday", -3.120895416507997),
                                    ("<integer> <unit-of-duration>time-of-day (latent)",
                                     -3.5263605246161616),
                                    ("a <unit-of-duration>noon|midnight|EOD|end of day",
                                     -3.120895416507997),
                                    ("<integer> <unit-of-duration><day-of-month> (ordinal or number) <named-month>",
                                     -3.5263605246161616)],
                               n = 21}}),
       ("quarter after|past <hour-of-day>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -3.1780538303479458,
                               likelihoods =
                                 HashMap.fromList
                                   [("noon|midnight|EOD|end of day", -2.03688192726104),
                                    ("time-of-day (latent)", -0.9382696385929302),
                                    ("hour", -0.7375989431307791)],
                               n = 10},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("yyyy-mm",
        Classifier{okData =
                     ClassData{prior = -1.0986122886681098,
                               unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -0.40546510810816444,
                               unseen = -1.791759469228055,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 4}}),
       ("until <time>",
        Classifier{okData =
                     ClassData{prior = -2.0794415416798357, unseen = -3.332204510175204,
                               likelihoods =
                                 HashMap.fromList
                                   [("year (latent)", -2.6026896854443837),
                                    ("time-of-day (latent)", -2.6026896854443837),
                                    ("year", -2.6026896854443837),
                                    ("<time-of-day> am|pm", -2.1972245773362196),
                                    ("hh:mm", -2.6026896854443837), ("hour", -2.1972245773362196),
                                    ("minute", -2.1972245773362196)],
                               n = 5},
                   koData =
                     ClassData{prior = -0.13353139262452263,
                               unseen = -4.477336814478207,
                               likelihoods =
                                 HashMap.fromList
                                   [("July", -3.7727609380946383),
                                    ("intersect", -2.856470206220483),
                                    ("yesterday", -3.367295829986474), ("day", -1.6933193964148026),
                                    ("the <day-of-month> (ordinal)", -3.7727609380946383),
                                    ("the <day-of-month> (number)", -3.7727609380946383),
                                    ("June", -3.079613757534693),
                                    ("time-of-day (latent)", -2.386466576974748),
                                    ("<time-of-day> am|pm", -3.7727609380946383),
                                    ("hh:mm", -2.856470206220483), ("hour", -2.1633230256605382),
                                    ("month", -2.856470206220483),
                                    ("at the beginning|end of <named-month>", -2.068012845856213),
                                    ("<named-month> <day-of-month> (non ordinal)",
                                     -3.7727609380946383),
                                    ("minute", -2.386466576974748)],
                               n = 35}}),
       ("the <cycle> after|before <time>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.0794415416798357,
                               likelihoods =
                                 HashMap.fromList
                                   [("day (grain)tomorrow", -1.252762968495368),
                                    ("dayday", -0.8472978603872037),
                                    ("day (grain)yesterday", -1.252762968495368)],
                               n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("about|exactly <time-of-day>",
        Classifier{okData =
                     ClassData{prior = -6.899287148695143e-2,
                               unseen = -3.7612001156935624,
                               likelihoods =
                                 HashMap.fromList
                                   [("week", -1.9459101490553135),
                                    ("hh(:mm) - <time-of-day> am|pm", -3.044522437723423),
                                    ("this|last|next <cycle>", -1.9459101490553135),
                                    ("day", -2.639057329615259),
                                    ("time-of-day (latent)", -3.044522437723423),
                                    ("hhmm (latent)", -3.044522437723423),
                                    ("<time-of-day> am|pm", -2.639057329615259),
                                    ("hh:mm", -3.044522437723423), ("hour", -2.3513752571634776),
                                    ("next <time>", -3.044522437723423),
                                    ("hhmm (military) am|pm", -3.044522437723423),
                                    ("this|next <day-of-week>", -3.044522437723423),
                                    ("minute", -2.128231705849268)],
                               n = 14},
                   koData =
                     ClassData{prior = -2.70805020110221, unseen = -2.833213344056216,
                               likelihoods =
                                 HashMap.fromList
                                   [("dd/mm", -2.0794415416798357), ("day", -2.0794415416798357)],
                               n = 1}}),
       ("Sukkot",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<day> in <duration>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.4849066497880004,
                               likelihoods =
                                 HashMap.fromList
                                   [("monthyear", -1.2992829841302609),
                                    ("Marcha <unit-of-duration>", -1.7047480922384253),
                                    ("March<integer> <unit-of-duration>", -1.7047480922384253),
                                    ("Vijayadashami<integer> <unit-of-duration>",
                                     -1.7047480922384253),
                                    ("dayyear", -1.7047480922384253)],
                               n = 3},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.791759469228055,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("World Vegan Day",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("since <time>",
        Classifier{okData =
                     ClassData{prior = -0.45198512374305727,
                               unseen = -3.295836866004329,
                               likelihoods =
                                 HashMap.fromList
                                   [("January", -2.5649493574615367), ("July", -2.5649493574615367),
                                    ("intersect", -2.5649493574615367),
                                    ("year (latent)", -2.5649493574615367),
                                    ("day", -2.5649493574615367),
                                    ("time-of-day (latent)", -2.5649493574615367),
                                    ("year", -2.5649493574615367),
                                    ("<time-of-day> am|pm", -2.5649493574615367),
                                    ("hour", -2.159484249353372), ("month", -1.8718021769015913),
                                    ("<named-month> <day-of-month> (non ordinal)",
                                     -2.5649493574615367)],
                               n = 7},
                   koData =
                     ClassData{prior = -1.0116009116784799, unseen = -3.044522437723423,
                               likelihoods =
                                 HashMap.fromList
                                   [("intersect", -1.8971199848858813),
                                    ("day", -1.3862943611198906),
                                    ("<datetime> - <datetime> (interval)", -1.8971199848858813)],
                               n = 4}}),
       ("intersect by \",\", \"of\", \"from\", \"'s\"",
        Classifier{okData =
                     ClassData{prior = -0.5335170349680614, unseen = -5.198497031265826,
                               likelihoods =
                                 HashMap.fromList
                                   [("Wednesday<named-month> <day-of-month> (non ordinal)",
                                     -4.499809670330265),
                                    ("dayhour", -4.0943445622221),
                                    ("daymonth", -2.4849066497880004),
                                    ("<named-month> <day-of-month> (non ordinal)Friday",
                                     -4.499809670330265),
                                    ("Friday<named-month> <day-of-month> (non ordinal)",
                                     -3.247046701834897),
                                    ("Wednesdayintersect", -4.499809670330265),
                                    ("from <datetime> - <datetime> (interval)July",
                                     -3.1135153092103742),
                                    ("Black Fridaythis|last|next <cycle>", -4.499809670330265),
                                    ("<part-of-day> of <time>February", -4.499809670330265),
                                    ("Saturday<time-of-day> am|pm", -4.499809670330265),
                                    ("Martin Luther King's Daythis|last|next <cycle>",
                                     -4.0943445622221),
                                    ("hourmonth", -4.499809670330265),
                                    ("Fridayintersect by \",\", \"of\", \"from\" for year",
                                     -3.8066624897703196),
                                    ("dayday", -2.2485178717237697),
                                    ("WednesdayOctober", -4.499809670330265),
                                    ("Wednesdaythis|last|next <cycle>", -4.0943445622221),
                                    ("intersect<named-month> <day-of-month> (non ordinal)",
                                     -3.247046701834897),
                                    ("dayyear", -3.8066624897703196),
                                    ("Saturday<named-month> <day-of-month> (non ordinal)",
                                     -4.499809670330265),
                                    ("Thursdayhh:mm", -4.0943445622221),
                                    ("TuesdayOctober", -4.499809670330265),
                                    ("Mondaythis|last|next <cycle>", -3.8066624897703196),
                                    ("Fridayintersect", -3.8066624897703196),
                                    ("Thursday<datetime> - <datetime> (interval)",
                                     -4.0943445622221),
                                    ("Thursday<time-of-day> - <time-of-day> (interval)",
                                     -3.58351893845611),
                                    ("Tuesdaythis|last|next <cycle>", -4.0943445622221),
                                    ("Sunday<named-month> <day-of-month> (non ordinal)",
                                     -4.499809670330265),
                                    ("dayminute", -2.70805020110221),
                                    ("minuteday", -3.247046701834897),
                                    ("this|last|next <cycle>Sunday", -4.499809670330265),
                                    ("Sundaythis|last|next <cycle>", -4.499809670330265),
                                    ("on <day><time-of-day> am|pm", -4.499809670330265),
                                    ("weekday", -4.499809670330265),
                                    ("dayweek", -3.4011973816621555),
                                    ("between <time> and <time>July", -4.499809670330265),
                                    ("Monday<named-month> <day-of-month> (non ordinal)",
                                     -4.0943445622221),
                                    ("on <day><named-month> <day-of-month> (non ordinal)",
                                     -3.8066624897703196)],
                               n = 61},
                   koData =
                     ClassData{prior = -0.8831907834478103, unseen = -4.976733742420574,
                               likelihoods =
                                 HashMap.fromList
                                   [("week-endJuly", -4.276666119016055),
                                    ("week-endOctober", -3.58351893845611),
                                    ("daymonth", -1.791759469228055),
                                    ("TuesdaySeptember", -4.276666119016055),
                                    ("Wednesdayintersect", -4.276666119016055),
                                    ("from <datetime> - <datetime> (interval)July",
                                     -3.1780538303479458),
                                    ("from <time-of-day> - <time-of-day> (interval)July",
                                     -4.276666119016055),
                                    ("hourmonth", -2.4849066497880004),
                                    ("Fridaythis|last|next <cycle>", -4.276666119016055),
                                    ("SundayFebruary", -4.276666119016055),
                                    ("on <day>September", -3.58351893845611),
                                    ("WednesdayOctober", -4.276666119016055),
                                    ("week-endintersect", -4.276666119016055),
                                    ("dayyear", -4.276666119016055),
                                    ("FridayJuly", -3.3603753871419),
                                    ("FridaySeptember", -3.871201010907891),
                                    ("WednesdayFebruary", -4.276666119016055),
                                    ("minutemonth", -2.890371757896165),
                                    ("Mondaythis|last|next <cycle>", -4.276666119016055),
                                    ("SundayMarch", -4.276666119016055),
                                    ("MondayFebruary", -3.871201010907891),
                                    ("Fridayintersect", -4.276666119016055),
                                    ("dayminute", -4.276666119016055),
                                    ("SaturdaySeptember", -4.276666119016055),
                                    ("intersectSeptember", -3.0239031505206873),
                                    ("MondayMarch", -4.276666119016055),
                                    ("FridayOctober", -4.276666119016055),
                                    ("<integer> to|till|before <hour-of-day>July",
                                     -4.276666119016055),
                                    ("Tuesdayintersect", -4.276666119016055),
                                    ("Sundayintersect", -4.276666119016055)],
                               n = 43}}),
       ("last <time>",
        Classifier{okData =
                     ClassData{prior = -0.6931471805599453,
                               unseen = -3.4339872044851463,
                               likelihoods =
                                 HashMap.fromList
                                   [("Martin Luther King's Day", -2.70805020110221),
                                    ("January", -2.70805020110221), ("day", -1.3217558399823195),
                                    ("Sunday", -2.70805020110221),
                                    ("Chinese New Year", -2.0149030205422647),
                                    ("Easter Sunday", -2.70805020110221),
                                    ("hour", -2.70805020110221), ("month", -2.70805020110221),
                                    ("Tuesday", -2.70805020110221),
                                    ("week-end", -2.70805020110221)],
                               n = 9},
                   koData =
                     ClassData{prior = -0.6931471805599453,
                               unseen = -3.4339872044851463,
                               likelihoods =
                                 HashMap.fromList
                                   [("Monday", -2.70805020110221), ("Friday", -2.3025850929940455),
                                    ("day", -1.791759469228055), ("Sunday", -2.70805020110221),
                                    ("hour", -1.6094379124341003),
                                    ("week-end", -1.6094379124341003)],
                               n = 9}}),
       ("March",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -3.4657359027997265,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 30},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<named-month>|<named-day> <day-of-month> (ordinal)",
        Classifier{okData =
                     ClassData{prior = -8.004270767353637e-2,
                               unseen = -3.5553480614894135,
                               likelihoods =
                                 HashMap.fromList
                                   [("Octoberordinal (digits)", -2.1400661634962708),
                                    ("Julyordinal (digits)", -2.4277482359480516),
                                    ("Thursdayordinal (digits)", -2.833213344056216),
                                    ("day", -2.4277482359480516),
                                    ("Augustordinal (digits)", -2.833213344056216),
                                    ("Marchordinals (first..twentieth,thirtieth,...)",
                                     -2.4277482359480516),
                                    ("Tuesdayordinal (digits)", -2.833213344056216),
                                    ("Octoberordinals (first..twentieth,thirtieth,...)",
                                     -2.833213344056216),
                                    ("month", -1.128465251817791),
                                    ("Marchordinal (digits)", -2.833213344056216)],
                               n = 12},
                   koData =
                     ClassData{prior = -2.5649493574615367,
                               unseen = -2.5649493574615367,
                               likelihoods =
                                 HashMap.fromList
                                   [("Augustordinal (digits)", -1.791759469228055),
                                    ("month", -1.791759469228055)],
                               n = 1}}),
       ("at the beginning|start|end of <week>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -4.330733340286331,
                               likelihoods =
                                 HashMap.fromList
                                   [("week", -0.706570200892086),
                                    ("this|last|next <cycle>", -0.8209805520698302),
                                    ("about|exactly <time-of-day>", -2.70805020110221)],
                               n = 36},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Clean Monday",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<day-of-month> (ordinal)",
        Classifier{okData =
                     ClassData{prior = -0.1670540846631662,
                               unseen = -2.5649493574615367,
                               likelihoods = HashMap.fromList [("ordinal (digits)", 0.0)],
                               n = 11},
                   koData =
                     ClassData{prior = -1.8718021769015913,
                               unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("ordinal (digits)", 0.0)],
                               n = 2}}),
       ("<time> (timezone)",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.6094379124341003,
                               likelihoods =
                                 HashMap.fromList
                                   [("<time-of-day> am|pm", -0.6931471805599453),
                                    ("hour", -0.6931471805599453)],
                               n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("qq",
        Classifier{okData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0},
                   koData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2}}),
       ("Easter Sunday",
        Classifier{okData =
                     ClassData{prior = -0.2876820724517809,
                               unseen = -2.0794415416798357,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 6},
                   koData =
                     ClassData{prior = -1.3862943611198906,
                               unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2}}),
       ("Christmas",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.70805020110221,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 13},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<datetime> - <datetime> (interval) timezone",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.3025850929940455,
                               likelihoods =
                                 HashMap.fromList
                                   [("minuteminute", -0.8109302162163288),
                                    ("hh:mmhh:mm", -1.5040773967762742),
                                    ("hhhmmhhhmm", -1.0986122886681098)],
                               n = 3},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Isra and Mi'raj",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.791759469228055,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 4},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<day-of-month> (ordinal or number) of <month>",
        Classifier{okData =
                     ClassData{prior = -0.41616039722491244,
                               unseen = -4.343805421853684,
                               likelihoods =
                                 HashMap.fromList
                                   [("ordinal (digits)July", -3.6375861597263857),
                                    ("ordinal (digits)January", -3.6375861597263857),
                                    ("ordinals (first..twentieth,thirtieth,...)last <time>",
                                     -3.6375861597263857),
                                    ("ordinals (first..twentieth,thirtieth,...)March",
                                     -2.384823191231018),
                                    ("ordinal (digits)this|last|next <cycle>", -2.384823191231018),
                                    ("integer (numeric)this|last|next <cycle>",
                                     -3.2321210516182215),
                                    ("ordinal (digits)intersect", -3.2321210516182215),
                                    ("ordinal (digits)February", -3.2321210516182215),
                                    ("integer (numeric)February", -3.2321210516182215),
                                    ("month", -0.8649974374866046),
                                    ("ordinals (first..twentieth,thirtieth,...)this|last|next <cycle>",
                                     -2.7212954278522306),
                                    ("ordinal (digits)March", -3.2321210516182215),
                                    ("ordinals (first..twentieth,thirtieth,...)February",
                                     -3.6375861597263857),
                                    ("integer (numeric)July", -3.6375861597263857)],
                               n = 31},
                   koData =
                     ClassData{prior = -1.0775588794702773,
                               unseen = -3.8501476017100584,
                               likelihoods =
                                 HashMap.fromList
                                   [("ordinal (digits)July", -1.5260563034950494),
                                    ("ordinal (digits)February", -3.1354942159291497),
                                    ("month", -0.995428052432879),
                                    ("integer (numeric)July", -1.8827312474337816)],
                               n = 16}}),
       ("decimal number",
        Classifier{okData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0},
                   koData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1}}),
       ("Naraka Chaturdashi",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("yyyyqq",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("beginning of month",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.0794415416798357,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 6},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("next <time>",
        Classifier{okData =
                     ClassData{prior = -0.1823215567939546,
                               unseen = -3.4339872044851463,
                               likelihoods =
                                 HashMap.fromList
                                   [("Martin Luther King's Day", -2.0149030205422647),
                                    ("Halloween", -2.70805020110221),
                                    ("Boss's Day", -2.70805020110221),
                                    ("Monday", -2.3025850929940455), ("day", -1.0986122886681098),
                                    ("March", -2.70805020110221), ("month", -2.70805020110221),
                                    ("Tuesday", -2.3025850929940455)],
                               n = 10},
                   koData =
                     ClassData{prior = -1.791759469228055, unseen = -2.70805020110221,
                               likelihoods =
                                 HashMap.fromList
                                   [("Wednesday", -1.9459101490553135),
                                    ("Saturday", -1.9459101490553135), ("day", -1.540445040947149)],
                               n = 2}}),
       ("<time-of-day> sharp|exactly",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.890371757896165,
                               likelihoods =
                                 HashMap.fromList
                                   [("at <time-of-day>", -2.1400661634962708),
                                    ("time-of-day (latent)", -2.1400661634962708),
                                    ("hhmm (latent)", -2.1400661634962708),
                                    ("<time-of-day> am|pm", -2.1400661634962708),
                                    ("hh:mm", -2.1400661634962708), ("hour", -1.7346010553881064),
                                    ("minute", -1.4469189829363254)],
                               n = 5},
                   koData =
                     ClassData{prior = -infinity, unseen = -2.0794415416798357,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("the <ordinal> closest <day> to <time>",
        Classifier{okData =
                     ClassData{prior = -0.2876820724517809,
                               unseen = -2.5649493574615367,
                               likelihoods =
                                 HashMap.fromList
                                   [("ordinals (first..twentieth,thirtieth,...)Monday<named-month>|<named-day> <day-of-month> (ordinal)",
                                     -1.791759469228055),
                                    ("ordinals (first..twentieth,thirtieth,...)Christmastoday",
                                     -1.791759469228055),
                                    ("dayday", -1.0986122886681098),
                                    ("ordinal (digits)Christmastoday", -1.791759469228055)],
                               n = 3},
                   koData =
                     ClassData{prior = -1.3862943611198906,
                               unseen = -2.1972245773362196,
                               likelihoods =
                                 HashMap.fromList
                                   [("daymonth", -1.3862943611198906),
                                    ("ordinals (first..twentieth,thirtieth,...)MondayOctober",
                                     -1.3862943611198906)],
                               n = 1}}),
       ("Islamic New Year",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Lent",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("negative numbers",
        Classifier{okData =
                     ClassData{prior = -infinity, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [], n = 0},
                   koData =
                     ClassData{prior = 0.0, unseen = -3.713572066704308,
                               likelihoods =
                                 HashMap.fromList
                                   [("integer (numeric)", -5.129329438755058e-2),
                                    ("integer (0..19)", -2.995732273553991)],
                               n = 38}}),
       ("about|exactly <duration>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.6094379124341003,
                               likelihoods =
                                 HashMap.fromList
                                   [("half a <time-grain>", -0.6931471805599453),
                                    ("minute", -0.6931471805599453)],
                               n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Purim",
        Classifier{okData =
                     ClassData{prior = -0.6931471805599453,
                               unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -0.6931471805599453,
                               unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1}}),
       ("<time> before last|after next",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.639057329615259,
                               likelihoods =
                                 HashMap.fromList
                                   [("Wednesday", -1.8718021769015913),
                                    ("Friday", -1.466337068793427), ("day", -1.1786549963416462),
                                    ("March", -1.8718021769015913), ("month", -1.8718021769015913)],
                               n = 4},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.791759469228055,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("by the end of <time>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.3025850929940455,
                               likelihoods =
                                 HashMap.fromList
                                   [("this|last|next <cycle>", -0.8109302162163288),
                                    ("year", -1.5040773967762742), ("month", -1.0986122886681098)],
                               n = 3},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("half an hour (abbrev).",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("hhmm (military) am|pm",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<datetime> - <datetime> (interval)",
        Classifier{okData =
                     ClassData{prior = -0.6716409753389816, unseen = -4.890349128221754,
                               likelihoods =
                                 HashMap.fromList
                                   [("daymonth", -3.4965075614664802),
                                    ("monthday", -2.580216829592325),
                                    ("at the beginning|end of <named-month>June",
                                     -3.4965075614664802),
                                    ("minuteminute", -1.9924301646902063),
                                    ("<time> timezone<time> timezone", -3.4965075614664802),
                                    ("hh:mmhh:mm", -2.6855773452501515),
                                    ("dayday", -2.3178525651248343),
                                    ("monthmonth", -3.784189633918261),
                                    ("hhhmmhhhmm", -3.2733640101522705),
                                    ("hourhour", -3.784189633918261),
                                    ("<named-month> <day-of-month> (non ordinal)<named-month> <day-of-month> (non ordinal)",
                                     -3.0910424533583156),
                                    ("intersect by \",\", \"of\", \"from\", \"'s\"hh:mm",
                                     -3.784189633918261),
                                    ("<time-of-day> am|pmhh:mm:ss", -4.189654742026425),
                                    ("at the beginning|end of <named-month>at the beginning|end of <named-month>",
                                     -2.9368917735310576),
                                    ("hoursecond", -3.784189633918261),
                                    ("<time-of-day> am|pm<time-of-day> am|pm", -3.4965075614664802),
                                    ("since <time><named-month> <day-of-month> (non ordinal)",
                                     -4.189654742026425),
                                    ("Mayat the beginning|end of <named-month>",
                                     -2.580216829592325),
                                    ("MayJune", -3.784189633918261)],
                               n = 47},
                   koData =
                     ClassData{prior = -0.7151260872787205, unseen = -4.859812404361672,
                               likelihoods =
                                 HashMap.fromList
                                   [("hhhmm<time> timezone", -3.7534179752515073),
                                    ("<time> timezonehhhmm", -3.7534179752515073),
                                    ("<named-month> <day-of-month> (non ordinal)July",
                                     -3.242592351485517),
                                    ("daymonth", -2.6548056865833978),
                                    ("<time-of-day> am|pm<time-of-day> - <time-of-day> (interval)",
                                     -3.7534179752515073),
                                    ("monthday", -3.4657359027997265),
                                    ("<time-of-day> am|pmintersect", -4.1588830833596715),
                                    ("MondayOctober", -3.7534179752515073),
                                    ("hh:mm<time-of-day> am|pm", -3.0602707946915624),
                                    ("hhhmmintersect", -4.1588830833596715),
                                    ("minuteminute", -1.9075912847531766),
                                    ("hh:mmhh:mm", -3.4657359027997265),
                                    ("dayday", -2.906120114864304),
                                    ("monthmonth", -3.4657359027997265),
                                    ("hhhmmhhhmm", -3.7534179752515073),
                                    ("hourhour", -4.1588830833596715),
                                    ("hourminute", -3.4657359027997265),
                                    ("minutehour", -3.4657359027997265),
                                    ("<time> timezonehh:mm", -4.1588830833596715),
                                    ("hh:mm<time> timezone", -4.1588830833596715),
                                    ("hh:mmintersect", -3.242592351485517),
                                    ("since <time>July", -4.1588830833596715),
                                    ("<named-month> <day-of-month> (non ordinal)August",
                                     -4.1588830833596715),
                                    ("Christmastoday", -3.4657359027997265),
                                    ("Monday<named-month>|<named-day> <day-of-month> (ordinal)",
                                     -3.7534179752515073),
                                    ("<time-of-day> am|pmhh:mm", -4.1588830833596715),
                                    ("Monday<named-month> <day-of-month> (non ordinal)",
                                     -4.1588830833596715),
                                    ("Mayat the beginning|end of <named-month>",
                                     -3.4657359027997265),
                                    ("MayJune", -3.4657359027997265)],
                               n = 45}}),
       ("Tuesday",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.772588722239781,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 14},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("New Year's Day",
        Classifier{okData =
                     ClassData{prior = -1.8718021769015913,
                               unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -0.1670540846631662,
                               unseen = -2.5649493574615367,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 11}}),
       ("fortnight",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<integer> and an half hour",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("integer (numeric)", 0.0)],
                               n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Laylat al-Qadr",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.791759469228055,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 4},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Boghi",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("at the beginning|end of <named-month>",
        Classifier{okData =
                     ClassData{prior = -2.6668247082161294e-2,
                               unseen = -4.394449154672439,
                               likelihoods =
                                 HashMap.fromList
                                   [("January", -3.283414346005772), ("May", -1.817077277212345),
                                    ("April", -3.283414346005772), ("June", -1.2909841813155656),
                                    ("month", -0.7444404749474959)],
                               n = 37},
                   koData =
                     ClassData{prior = -3.6375861597263857,
                               unseen = -2.1972245773362196,
                               likelihoods =
                                 HashMap.fromList
                                   [("October", -1.3862943611198906),
                                    ("month", -1.3862943611198906)],
                               n = 1}}),
       ("<time-of-day> - <time-of-day> (interval)",
        Classifier{okData =
                     ClassData{prior = -1.1468143904859875, unseen = -4.394449154672439,
                               likelihoods =
                                 HashMap.fromList
                                   [("hh:mmtime-of-day (latent)", -2.995732273553991),
                                    ("minuteminute", -1.6094379124341003),
                                    ("<time> timezone<time> timezone", -2.995732273553991),
                                    ("hh:mmhh:mm", -2.184802057337662),
                                    ("<time-of-day> am|pmtime-of-day (latent)", -3.283414346005772),
                                    ("hhhmmhhhmm", -2.772588722239781),
                                    ("hourhour", -2.772588722239781),
                                    ("minutehour", -2.436116485618568),
                                    ("<time-of-day> am|pmhh:mm:ss", -3.6888794541139363),
                                    ("hhhmmtime-of-day (latent)", -2.995732273553991),
                                    ("hoursecond", -3.283414346005772),
                                    ("<time-of-day> am|pm<time-of-day> am|pm", -2.995732273553991)],
                               n = 27},
                   koData =
                     ClassData{prior = -0.3822082459438971, unseen = -4.962844630259907,
                               likelihoods =
                                 HashMap.fromList
                                   [("hhhmm<time> timezone", -3.857214768933151),
                                    ("<time> timezonehhhmm", -3.857214768933151),
                                    ("until <time>time-of-day (latent)", -3.164067588373206),
                                    ("hh:mmtime-of-day (latent)", -2.1832383353614793),
                                    ("hh:mm<time-of-day> am|pm", -2.8763855159214247),
                                    ("minuteminute", -2.316769727986002),
                                    ("hh:mmhh:mm", -3.56953269648137),
                                    ("<time-of-day> am|pmtime-of-day (latent)", -3.857214768933151),
                                    ("hhhmmhhhmm", -3.857214768933151),
                                    ("hourhour", -2.247776856499051),
                                    ("hourminute", -4.2626798770413155),
                                    ("minutehour", -1.5218398531161146),
                                    ("<time> timezonehh:mm", -4.2626798770413155),
                                    ("hh:mm<time> timezone", -4.2626798770413155),
                                    ("hhhmmtime-of-day (latent)", -3.56953269648137),
                                    ("until <time><time-of-day> am|pm", -4.2626798770413155),
                                    ("<time-of-day> am|pmhh:mm", -4.2626798770413155),
                                    ("<part-of-day> at <time-of-day>time-of-day (latent)",
                                     -3.56953269648137),
                                    ("at <time-of-day>time-of-day (latent)", -3.56953269648137),
                                    ("<time> timezonetime-of-day (latent)", -3.56953269648137),
                                    ("<integer> to|till|before <hour-of-day><time-of-day> am|pm",
                                     -4.2626798770413155),
                                    ("<integer> to|till|before <hour-of-day>time-of-day (latent)",
                                     -3.56953269648137)],
                               n = 58}}),
       ("military spelled out numbers am|pm",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods =
                                 HashMap.fromList [("integer (0..19)integer (20..90)", 0.0)],
                               n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("winter",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("nth <time> after <time>",
        Classifier{okData =
                     ClassData{prior = -0.6931471805599453, unseen = -1.791759469228055,
                               likelihoods =
                                 HashMap.fromList
                                   [("dayday", -0.916290731874155),
                                    ("ordinals (first..twentieth,thirtieth,...)Tuesdayintersect",
                                     -0.916290731874155)],
                               n = 1},
                   koData =
                     ClassData{prior = -0.6931471805599453, unseen = -1.791759469228055,
                               likelihoods =
                                 HashMap.fromList
                                   [("dayday", -0.916290731874155),
                                    ("ordinals (first..twentieth,thirtieth,...)TuesdayChristmas",
                                     -0.916290731874155)],
                               n = 1}}),
       ("Ugadi",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.1972245773362196,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 7},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<named-month> <day-of-month> (non ordinal)",
        Classifier{okData =
                     ClassData{prior = -0.2732933349996813, unseen = -4.394449154672439,
                               likelihoods =
                                 HashMap.fromList
                                   [("Augustinteger (numeric)", -2.995732273553991),
                                    ("since <time>integer (numeric)", -3.6888794541139363),
                                    ("Marchinteger (numeric)", -3.283414346005772),
                                    ("Aprilinteger (numeric)", -3.6888794541139363),
                                    ("month", -0.7985076962177716),
                                    ("Februaryinteger (numeric)", -2.436116485618568),
                                    ("Septemberinteger (numeric)", -2.995732273553991),
                                    ("Octoberinteger (numeric)", -2.995732273553991),
                                    ("Julyinteger (numeric)", -1.5488132906176655)],
                               n = 35},
                   koData =
                     ClassData{prior = -1.4307461236907244,
                               unseen = -3.4965075614664802,
                               likelihoods =
                                 HashMap.fromList
                                   [("Augustinteger (numeric)", -2.367123614131617),
                                    ("Marchinteger (numeric)", -2.772588722239781),
                                    ("until <time>integer (numeric)", -2.772588722239781),
                                    ("Aprilinteger (numeric)", -2.772588722239781),
                                    ("month", -0.9808292530117262),
                                    ("Julyinteger (numeric)", -1.5198257537444133)],
                               n = 11}}),
       ("Diwali",
        Classifier{okData =
                     ClassData{prior = -0.2876820724517809,
                               unseen = -1.6094379124341003,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 3},
                   koData =
                     ClassData{prior = -1.3862943611198906,
                               unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1}}),
       ("last night",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("this|next <day-of-week>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -3.1354942159291497,
                               likelihoods =
                                 HashMap.fromList
                                   [("Thursday", -2.3978952727983707),
                                    ("Wednesday", -2.3978952727983707),
                                    ("Saturday", -2.3978952727983707),
                                    ("Monday", -1.7047480922384253), ("day", -0.8938178760220964),
                                    ("Tuesday", -1.9924301646902063)],
                               n = 8},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.9459101490553135,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("ordinal (digits)",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -4.304065093204169,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 72},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("quarter (grain)",
        Classifier{okData =
                     ClassData{prior = -0.4700036292457356,
                               unseen = -3.0910424533583156,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 20},
                   koData =
                     ClassData{prior = -0.9808292530117262, unseen = -2.639057329615259,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 12}}),
       ("Chhath",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.6094379124341003,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 3},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Vasant Panchami",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<day-of-month>(ordinal) <named-month> year",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.6094379124341003,
                               likelihoods =
                                 HashMap.fromList
                                   [("ordinal (digits)April", -0.6931471805599453),
                                    ("month", -0.6931471805599453)],
                               n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("the <ordinal> last <cycle> of <time>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.9444389791664407,
                               likelihoods =
                                 HashMap.fromList
                                   [("ordinals (first..twentieth,thirtieth,...)week (grain)year (latent)",
                                     -2.1972245773362196),
                                    ("daymonth", -2.1972245773362196),
                                    ("ordinal (digits)day (grain)May", -2.1972245773362196),
                                    ("ordinals (first..twentieth,thirtieth,...)week (grain)intersect",
                                     -2.1972245773362196),
                                    ("weekmonth", -1.791759469228055),
                                    ("ordinal (digits)week (grain)year (latent)",
                                     -2.1972245773362196),
                                    ("weekyear", -1.791759469228055),
                                    ("ordinals (first..twentieth,thirtieth,...)week (grain)October",
                                     -2.1972245773362196)],
                               n = 5},
                   koData =
                     ClassData{prior = -infinity, unseen = -2.1972245773362196,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Black Friday",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.791759469228055,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 4},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("week-end",
        Classifier{okData =
                     ClassData{prior = -1.252762968495368, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -0.3364722366212129,
                               unseen = -1.9459101490553135,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 5}}),
       ("Great Lent",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Maundy Thursday",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.6094379124341003,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 3},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("day (grain)",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -3.6888794541139363,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 38},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Vaisakhi",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.791759469228055,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 4},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("right now",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("for <duration> from <time>",
        Classifier{okData =
                     ClassData{prior = -0.40546510810816444,
                               unseen = -2.3025850929940455,
                               likelihoods =
                                 HashMap.fromList
                                   [("<integer> <unit-of-duration><day-of-month> (ordinal)",
                                     -1.5040773967762742),
                                    ("dayday", -1.0986122886681098),
                                    ("<integer> <unit-of-duration><day-of-month> (ordinal or number) <named-month>",
                                     -1.5040773967762742)],
                               n = 2},
                   koData =
                     ClassData{prior = -1.0986122886681098,
                               unseen = -2.0794415416798357,
                               likelihoods =
                                 HashMap.fromList
                                   [("dayhour", -1.252762968495368),
                                    ("<integer> <unit-of-duration>time-of-day (latent)",
                                     -1.252762968495368)],
                               n = 1}}),
       ("compose by multiplication",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.6094379124341003,
                               likelihoods =
                                 HashMap.fromList [("integer (0..19)powers of tens", 0.0)],
                               n = 3},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("end of year",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.4849066497880004,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 10},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("between <time> and <time>",
        Classifier{okData =
                     ClassData{prior = -0.4795730802618862,
                               unseen = -3.9889840465642745,
                               likelihoods =
                                 HashMap.fromList
                                   [("daymonth", -3.2771447329921766),
                                    ("monthday", -2.871679624884012),
                                    ("at the beginning|end of <named-month>June",
                                     -3.2771447329921766),
                                    ("minuteminute", -2.871679624884012),
                                    ("hh:mmhh:mm", -3.2771447329921766),
                                    ("dayday", -2.3608540011180215),
                                    ("monthmonth", -3.2771447329921766),
                                    ("<time-of-day> am|pmtime-of-day (latent)",
                                     -3.2771447329921766),
                                    ("hhhmmhhhmm", -3.2771447329921766),
                                    ("<named-month> <day-of-month> (non ordinal)<named-month> <day-of-month> (non ordinal)",
                                     -3.2771447329921766),
                                    ("minutehour", -2.871679624884012),
                                    ("the <day-of-month> (ordinal)<day-of-month> (ordinal)",
                                     -3.2771447329921766),
                                    ("at the beginning|end of <named-month>at the beginning|end of <named-month>",
                                     -2.871679624884012),
                                    ("<time-of-day> am|pm<time-of-day> am|pm", -3.2771447329921766),
                                    ("yearday", -3.2771447329921766),
                                    ("year (latent)at the beginning|end of <named-month>",
                                     -3.2771447329921766),
                                    ("Mayat the beginning|end of <named-month>",
                                     -2.871679624884012),
                                    ("MayJune", -3.2771447329921766)],
                               n = 13},
                   koData =
                     ClassData{prior = -0.9650808960435872, unseen = -3.784189633918261,
                               likelihoods =
                                 HashMap.fromList
                                   [("<named-month> <day-of-month> (non ordinal)July",
                                     -3.068052935133617),
                                    ("the <day-of-month> (ordinal)<day-of-month> (ordinal or number) of <month>",
                                     -3.068052935133617),
                                    ("dayhour", -3.068052935133617),
                                    ("daymonth", -3.068052935133617),
                                    ("the <day-of-month> (ordinal)<day-of-month> (ordinal or number) of <named-month>",
                                     -3.068052935133617),
                                    ("hh:mmtime-of-day (latent)", -3.068052935133617),
                                    ("hhhmmintersect", -3.068052935133617),
                                    ("minuteminute", -2.662587827025453),
                                    ("dayday", -2.662587827025453),
                                    ("the <day-of-month> (ordinal)time-of-day (latent)",
                                     -3.068052935133617),
                                    ("minutehour", -2.662587827025453),
                                    ("hh:mmintersect", -3.068052935133617),
                                    ("hhhmmtime-of-day (latent)", -3.068052935133617)],
                               n = 8}}),
       ("<month> dd-dd (interval)",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -3.1354942159291497,
                               likelihoods =
                                 HashMap.fromList
                                   [("Julyinteger (numeric)integer (numeric)", -1.2992829841302609),
                                    ("Augustordinal (digits)integer (numeric)",
                                     -1.9924301646902063),
                                    ("month", -0.7884573603642702),
                                    ("Augustordinal (digits)ordinal (digits)",
                                     -1.9924301646902063)],
                               n = 9},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.6094379124341003,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("intersect by \",\", \"of\", \"from\" for year",
        Classifier{okData =
                     ClassData{prior = -7.410797215372185e-2,
                               unseen = -3.5263605246161616,
                               likelihoods =
                                 HashMap.fromList
                                   [("Black Fridaythis|last|next <cycle>", -2.803360380906535),
                                    ("Martin Luther King's Daythis|last|next <cycle>",
                                     -2.3978952727983707),
                                    ("intersect by \",\", \"of\", \"from\", \"'s\"year (latent)",
                                     -2.1102132003465894),
                                    ("dayyear", -0.8574502318512216),
                                    ("intersectyear (latent)", -2.1102132003465894),
                                    ("<named-month> <day-of-month> (non ordinal)year (latent)",
                                     -1.8870696490323797)],
                               n = 13},
                   koData =
                     ClassData{prior = -2.639057329615259, unseen = -2.3025850929940455,
                               likelihoods =
                                 HashMap.fromList
                                   [("Fridaythis|last|next <cycle>", -1.5040773967762742),
                                    ("dayyear", -1.5040773967762742)],
                               n = 1}}),
       ("part of days",
        Classifier{okData =
                     ClassData{prior = -4.1672696400568074e-2,
                               unseen = -3.891820298110627,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 47},
                   koData =
                     ClassData{prior = -3.1986731175506815,
                               unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2}}),
       ("Eid al-Fitr",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.1972245773362196,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 7},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("summer",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Trinity Sunday",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("<datetime>/<datetime> (interval)",
        Classifier{okData =
                     ClassData{prior = -1.791759469228055, unseen = -2.0794415416798357,
                               likelihoods =
                                 HashMap.fromList
                                   [("secondsecond", -1.252762968495368),
                                    ("intersectintersect", -1.252762968495368)],
                               n = 1},
                   koData =
                     ClassData{prior = -0.1823215567939546, unseen = -2.772588722239781,
                               likelihoods =
                                 HashMap.fromList
                                   [("hh:mm:ssintersect", -2.0149030205422647),
                                    ("minuteminute", -1.0986122886681098),
                                    ("secondsecond", -2.0149030205422647),
                                    ("intersectintersect", -1.6094379124341003),
                                    ("<time-of-day> - <time-of-day> (interval)intersect",
                                     -1.6094379124341003)],
                               n = 5}}),
       ("Mid-day",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.6094379124341003,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 3},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("dd-dd <month> (interval)",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -3.1354942159291497,
                               likelihoods =
                                 HashMap.fromList
                                   [("integer (numeric)integer (numeric)September",
                                     -1.9924301646902063),
                                    ("ordinal (digits)ordinal (digits)July", -1.9924301646902063),
                                    ("ordinal (digits)ordinal (digits)October",
                                     -2.3978952727983707),
                                    ("integer (numeric)integer (numeric)July", -1.9924301646902063),
                                    ("month", -0.8938178760220964),
                                    ("ordinal (digits)ordinal (digits)August",
                                     -2.3978952727983707)],
                               n = 8},
                   koData =
                     ClassData{prior = -infinity, unseen = -1.9459101490553135,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Pentecost",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.3862943611198906,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 2},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("this <time>",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -3.258096538021482,
                               likelihoods =
                                 HashMap.fromList
                                   [("Thursday", -2.5257286443082556),
                                    ("Martin Luther King's Day", -2.5257286443082556),
                                    ("Monday", -2.5257286443082556), ("day", -1.1394342831883648),
                                    ("Christmas", -2.5257286443082556),
                                    ("hour", -2.5257286443082556), ("winter", -2.5257286443082556),
                                    ("week-end", -2.5257286443082556),
                                    ("summer", -2.120263536200091)],
                               n = 8},
                   koData =
                     ClassData{prior = -infinity, unseen = -2.3025850929940455,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("Shushan Purim",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -1.0986122886681098,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 1},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}}),
       ("August",
        Classifier{okData =
                     ClassData{prior = 0.0, unseen = -2.0794415416798357,
                               likelihoods = HashMap.fromList [("", 0.0)], n = 6},
                   koData =
                     ClassData{prior = -infinity, unseen = -0.6931471805599453,
                               likelihoods = HashMap.fromList [], n = 0}})]